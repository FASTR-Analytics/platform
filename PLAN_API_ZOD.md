...
> **Status: IN PROGRESS — batch 1 (pilot + plumbing) DONE 2026-06-12** (reviewed: validation harness exercised bad-param/bad-body/missing-body/invalid-JSON/strip-mode against the live `defineRoute`; typecheck + boot green incl. the schema/path assertion). One reviewer fix landed: missing/invalid-JSON bodies originally bypassed the schema via the `c.req.json()` catch — now parsed with a `{}` fallback that is itself validated, so absent bodies 400 on required fields. Implementation notes for later batches: the `InferIfZod<T>` trick in `route-utils.ts` means schemas are runtime values typed as their inferred shapes — `server-action-types.ts` and the client needed zero changes; use `z.uuid()` (top-level), not the deprecated `z.string().uuid()`. The prerequisite hardening pass (PLAN_API_ROUTES_HARDENING, now deleted) landed in full on 2026-06-12: the 4xx-envelope client fix (its A1 — required because this plan introduces HTTP 400 responses), fail-hard boot validation, registry collision checks, `timeoutMs`, and the wire-space `TypedResponse<JSONParsed<…>>` handler-return constraint are all in place (see DOC_API_ROUTES.md). The "hardening A1/B3/B4/B6" references below refer to that landed work. Design agreed in the 2026-06-12 route-system review.
>
