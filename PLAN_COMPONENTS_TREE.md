...
**Next step: Review 10.** Each session sets this line in its final commit.
...
| 2026-09-22 | 9 | Starting point at `c602c8d0`: `snake-case` 93 (71 underscore-prefixed files, 21 PascalCase or camelCase files, the folder `ai_tools/tools/_internal/`), against the deliverable's 77 and 27 measured when the plan was written; steps 3 to 8 had renamed the rest. `git ls-files` counted 21 PascalCase, not 27, because six were `index.tsx` files steps 3 to 7 renamed. |
| 2026-09-22 | 9 | Choice the rulings did not cover: a folder for "one commit per folder" is the moved folder of steps 3 to 7 (`_shared/figure_editor`, `data/facilities`, `data/hfa`, `data/hmis`, `data/iceh`, `products/_shared`, `products/copilot`, `products/report`, `products/slide_deck`, `results_packages`, `users`), eleven groups in twelve commits. The figure editor took two (`3b7a08b5`, `5b4e861d`): its two panel folders moved whole first, so their three-line and seven-line `mod.ts` entries stayed byte-identical (a folder rename whose every entry line also changes shows the entry as `D` plus `A`, which G3 forbids); the files inside were renamed in the second commit, where the entries are `M`. |
| 2026-09-22 | 9 | Names chosen where R7's clauses meet: the HFA indicator files drop the `hfa_` prefix the table names and the folder's own word (`manager.tsx` for `HfaIndicatorsManager`, R7's example; `xlsx_upload_form.tsx`, `categories_manager.tsx`, `edit_category.tsx` and the other `edit_*.tsx`); the two wizards' steps are numbered in the order their `wizard.tsx` mounts them (`step_1_indicators`, `step_2_time`, `step_3_config`, `step_4_review`; `step_1_data`, `step_2_modules`, `step_3_confirm`); `_population_grid.tsx` is `grid.tsx` (`PopulationGrid` under `population/`); the HMIS import files that export `Dhis2*` take that prefix (`dhis2_run_detail`, `dhis2_tab_current`, and so on) while the `Csv*` files keep `csv_`; `_1_summary.tsx` is `summary.tsx` (two summary components, no single main export); `_shared.ts` under `hfa/indicators/` is `slugify.ts` (its one export); `_status_label.ts` is `run_status_label.ts` (`hfaRunStatusLabel`, `icehRunStatusLabel`); `tools/_internal/` is `format_for_ai/` (it holds `formatModulesListForAI` and `formatModuleSettingsForAI`); `viz_cursors.tsx` is `viz_editor_cursors.tsx` (`VizEditorCursors`). Beyond the lint and the table, `users/add_users.tsx` is `add_user_form.tsx`, since its export is `AddUserForm` and the deliverable makes a name that disagrees with its export a finding. |
| 2026-09-22 | 9 | Names kept, judged to agree with their exports: page files named for the page under R2 and R3 with the export kept under §6 (`dataset.tsx` for `InstanceDatasetHmis`, `report.tsx` for `ReportEditor`, `slide_deck.tsx` for `SlideDeckEditor`, `copilot.tsx` for `ProductCopilotHost`, `insert_figure.tsx`, `version_history.tsx`, `detail.tsx`, `prune.tsx`, `module_defaults.tsx`, `profile.tsx`, `save_report_style.tsx`); `figure_editor.tsx` for `VisualizationEditorInner`, which the table rules; `report_editor.tsx` for `ReportBodyEditor`; the three cursor files (`viz_editor_cursors`, `report_cursors`, `slide_cursors` for `*EditorCursors`, the per-surface cursor files S16 names); the `report_*` files under `report/`, which step 7 named and Review 7 passed, so R7's "never repeats its folder's name" was not applied to them; and function modules named for the noun they manage as R7 says of stores (`folder_tree`, `scroll_sync`, `rebase_edits`, `paginate_report`, `prune_plan`, `build_context_menu`, `version_diff`, `report_page_map`, `presence_guard`, `xlsx_workbook`, `computability`, `naming_step`, `type_facts`, `style_components`, `id_generation`, `module_graph`, `run_label`, `recurrence_label`, `column_labels`, `version_info`). |
| 2026-09-22 | 9 | Per-commit gates: the client typecheck, `lint:systems` and `lint:structure` over the folder ran before each of the twelve commits; the server check, the tests and the client build ran at the end, since no commit touches a file outside `client/src/components/`, the SYSTEM prose and the baseline. `validate_protocols_baseline.json` moved in `2c3b3fb1` (the figure embed) and `87c5d037` (the two prompt library modals and the documents store). |
| 2026-09-22 | 9 | G2 over the whole client with no scope: every check 0; `deno task lint:structure` prints OK. G3: every renamed file is an `R` row in its commit (132 rows, similarity 089 to 100, the lowest the three `editor_panel*.tsx` files whose many import lines changed in `3b7a08b5`), no `D`/`A` pair. G4: `./validate_protocols --update-baseline` leaves the file unchanged and the sorted `{id,text}` set is identical to `c602c8d0`'s (16 entries; four re-pathed). G5: `git grep` over `*.md` (excluding `PLAN_*.md` and `panther/`) is empty for every old path, and a bare-name sweep over the same docs and `lib/`, `server/`, `client/src/` comments finds none. No em-dash added. Floor green: `deno task typecheck`, `deno task test` (397 passed), `./validate_protocols`, `cd client && npm run build`. |
| 2026-09-22 | 9 | Step 9 built. |
| 2026-09-22 | 9 | Finding: R7's "a child never repeats its folder's name" was applied to `data/hfa/indicators/manager.tsx` and `data/hmis/population/grid.tsx` and left beside them fifteen names whose stem begins with the folder's own name and that no table row or §2 model line names: `data/hmis/indicators/indicators_manager.tsx` (`IndicatorsManager`, the shape of R7's own example), `data/hmis/population/population_manager.tsx`, `data/geojson/geojson_manager.tsx`, `data/geojson/geojson_edit_modal.tsx`, the folder `data/geojson/geojson_upload_wizard/`, `data/hfa/indicators/ai/ai_wrapper.tsx`, `data/hfa/indicators/_shared/hfa_r_code_validator.ts` (the table's `hfa_` prefix, under `indicators/_shared/`), `products/slide_deck/slide_deck_settings.tsx`, and the seven `products/report/report_*.tsx` (`report_cursors`, `report_editor`, `report_embed_editor`, `report_image_picker`, `report_markdown_diff`, `report_theme_modal`, `report_toolbar`). Row 719's reason for keeping the `report_*` files, that step 7 named them and Review 7 passed, is rejected: step 7's deliverable was placement, and R7 says the full list of names is step 9. Drop the leading folder word (`manager.tsx`, `settings.tsx`, `edit_modal.tsx`, `upload_wizard/`, `wrapper.tsx`, `r_code_validator.ts`, `cursors.tsx`, `body_editor.tsx` for `ReportBodyEditor`, `embed_editor.tsx`, `image_picker.tsx`, `markdown_diff.tsx`, `theme_modal.tsx`, `toolbar.tsx`); exports keep their names (§6). Docs that name them: `PROTOCOL_APP_UI_CONVENTIONS.md:119`, `SYSTEM_12_documents_sharing.md:400`, `:1345`, `:1373`, `SYSTEM_16_collaboration.md:479`, `:556`, `:1255`. |
| 2026-09-22 | 9 | Finding: `products/copilot/slide_ai/build_config_from_metric.ts` has one export, `buildConfigFromPreset` (line 22), and its name disagrees with it; no ruling and no entry of rows 718 or 719 covers it, so the deliverable's last sentence applies. Rename to `build_config_from_preset.ts`; `PROTOCOL_APP_AI_TOOLS.md:60` and `SYSTEM_13_ai_assistant.md:532` name the file. |
| 2026-09-22 | 9 | Finding: `products/slide_deck/style_editor/color_theme_utils.ts` has one export, `normalizeHex` (line 3), and its name is a bucket word plus a subject the export does not carry; the same sentence applies. Rename to `normalize_hex.ts`; no doc names it. |
| 2026-09-22 | 9 | Finding: `PLAN_COMPONENTS_TREE.md:721` says 132 `R` rows; `git diff -M --name-status <c>^ <c>` over the twelve commits gives 131 (21, 12, 1, 23, 32, 10, 2, 11, 2, 11, 5, 1), the same at `-M40%` and with `-C`. Similarity 089 to 100 and the three lowest files are right. Log accuracy only; no Fix follows from this row. |
| 2026-09-22 | 9 | Reviewer's reading, no change needed. Surface: the 167 changed files are the renamed files and their importers under `client/src/components/`, ten SYSTEM and PROTOCOL files whose prose or globs name them (`PROTOCOL_APP_STATE`, `PROTOCOL_APP_UI_CONVENTIONS`, `SYSTEM_05`, `_06`, `_07`, `_10`, `_11`, `_13`, `_15`, `_16`), the baseline and the plan; every changed code line is an import or re-export specifier except one comment in `visualization_editor.tsx` naming `viz_editor_cursors.tsx`, and no export changed name. Deliverable read in the tree: no underscore-prefixed file or folder but the `_shared/` folders, no mixed-case file, every table row realised (`figure_editor.tsx`, `editor_panel*`, `conditional_formatting_store.ts`, the `hfa_` prefixes under `indicators/` dropped, `edit_*.tsx`, `dataset_display_presentation.tsx` with the HFA and ICEH `dataset_items_holder.tsx` exporting `DatasetItemsHolder`, `ai_documents_store.ts`, `prompt_library_store.ts`, `draft_slide_preview.tsx`, `wrap_on_underscore.tsx`), the HMIS import and results package wizard steps numbered in the order their `wizard.tsx` mounts them. Name judgements in rows 718 and 719 accepted: the page files under R2 and R3 with exports kept under §6; `figure_editor.tsx`, `editor_panel*.tsx` and `conditional_formatting_store.ts` under the table; `summary.tsx`, `style_components.tsx`, `type_facts.tsx`, `treatment_picker.tsx` and the other modules with two or more exports, where no single main export exists; the single-function modules whose stem carries the export's noun (`column_labels` for `getStructureColumnLabel`, `version_info` for `fetchDatasetHmisVersion`, `report_page_map` for `describeReportPages`, `presence_guard` for `assertSlidesNotBusy`, `editor_snapshot` for `snapshotForSlideEditor`, `build_tools`, `fastr_fence_extension`, `client_info_topics`); `slide_cursors.tsx` for `SlideEditorCursors`, which does not repeat `slide_editor/`; and `viz_editor_cursors.tsx`. Also accepted, as outside the first finding above: names where the folder's word follows the export's own verb or noun rather than leading the stem (`share_report`, `download_report`, `download_slide_deck`, `share_slide_deck`, `delete_data`, `batch_upload_users_form`, `duplicate_products_modal`, `sort_indicators_modal`, `report_validators`), the reading the table's own `dataset_items_holder.tsx`, `dataset_display_presentation.tsx` and `ai_documents_store.ts` rows take; `hfa_weights.tsx` and `instance_meta_form.tsx`, which §2's model draws by those names; and the unnamed wizard steps `facilities/import/step_0.tsx` and `geojson_upload_wizard/step_{0,2,3,4}.tsx`, whose exports `Step0` to `Step4` carry no name to give the file, so a `step_N_name` form would make name and export disagree. Gates: G2 whole tree every check 0; G3 every renamed file an `R` row in its commit, no `D`/`A` pair, and the figure editor's two commits satisfy R8 and "one commit per folder" as a move commit and a follow-up, each free of an import naming a stem it renamed; G4 the sorted `{id,text}` set identical to `c602c8d0`'s and `--update-baseline` a no-op; G5 empty for every old path and every old stem in file-name context over `*.md` (excluding `PLAN_*.md` and `panther/`), `lib/`, `server/` and `client/src/`; no em-dash added. Row 716's counts verified from `git ls-tree` (71 underscore-prefixed files, 21 mixed-case files, `_internal/`); row 720's two baseline commits verified. Floor green at HEAD: `deno task typecheck`, `deno task test` (397 passed), `./validate_protocols`, `cd client && npm run build`. |
| 2026-09-22 | 9 | Step 9 reviewed: 4 findings. |
| 2026-09-22 | 9 | Fix: findings 1 to 3 applied in six commits, one per folder (`9c7fdd0c` HMIS, `722b4722` geojson, `e730c65e` HFA, `65198c4f` report, `51fbc0e6` slide deck, `3155c981` copilot): the fifteen names drop their leading folder word exactly as finding 1 lists them (`manager.tsx` under `hmis/indicators/`, `hmis/population/` and `geojson/`; `edit_modal.tsx` and `upload_wizard/` under `geojson/`; `ai/wrapper.tsx` and `_shared/r_code_validator.ts` under `hfa/indicators/`; `settings.tsx` under `slide_deck/`; `cursors`, `body_editor`, `embed_editor`, `image_picker`, `markdown_diff`, `theme_modal`, `toolbar` under `report/`), `build_config_from_metric.ts` is `build_config_from_preset.ts`, `color_theme_utils.ts` is `normalize_hex.ts`. Exports unchanged. Docs rewritten in the same commits: `PROTOCOL_APP_UI_CONVENTIONS.md:119`, `PROTOCOL_APP_AI_TOOLS.md:60`, `SYSTEM_12:400`, `:1345`, `:1373`, `SYSTEM_13:260`, `:532`, `:667`, `SYSTEM_16:479`, `:556`, `:1255`, the `lib/types/ai_input.ts` comments at lines 44 and 114, and the `products/_shared/report_html_preview.tsx:66` comment that named `report_cursors` (its pre-existing em-dash replaced while the line was rewritten). The two baseline entries for the geojson edit modal moved with it in `722b4722`; the entry set is unchanged. Finding 4 stands as the corrected count: 131 `R` rows in the twelve step commits. |
| 2026-09-22 | 9 | Fix gates: G2 whole tree OK, every check 0; G3 every renamed file an `R` row (24 rows, similarity 097 to 100, no `D`/`A`); G4 `--update-baseline` leaves the file unchanged and the sorted `{id,text}` set matches `c602c8d0`; G5 empty for the 24 old paths over `*.md` (excluding `PLAN_*.md`, `panther/`) and the code comments; no em-dash added. Floor green: `deno task typecheck`, `deno task test` (397 passed), `./validate_protocols`, `cd client && npm run build`. |
| 2026-09-22 | 9 | Step 9 fixed. |
| 2026-09-22 | 9 | Step 9 reviewed: pass. Re-review after Fix 9, scoped to what the six commits changed: `git diff --stat 9b31c924..HEAD` is the 24 renamed files and their importers under `client/src/components/`, five docs (`PROTOCOL_APP_AI_TOOLS`, `PROTOCOL_APP_UI_CONVENTIONS`, `SYSTEM_12`, `_13`, `_16`), the `lib/types/ai_input.ts` comments, the baseline and the plan, all inside the step's surface; every changed code line is an import or re-export specifier except the `report_html_preview.tsx:66` comment, which now names `report/cursors.tsx`. The harness over the renamed files finds none of the seventeen names findings 1 to 3 listed and no new disagreement: each renamed file's stem is its export minus the folder's word (`manager` for `IndicatorsManager`, `PopulationManager`, `GeoJsonManager`; `edit_modal`; `upload_wizard/`; `ai/wrapper`; `r_code_validator`; `settings`; `cursors`, `body_editor`, `embed_editor`, `image_picker`, `markdown_diff`, `theme_modal`, `toolbar`; `build_config_from_preset` for `buildConfigFromPreset`; `normalize_hex` for `normalizeHex`), exports unchanged. G2 whole tree every check 0; G3 24 `R` rows across the six commits (097 to 100), no `D`/`A`; G4 sorted `{id,text}` set identical to `c602c8d0`'s and `--update-baseline` a no-op; G5 empty for the 24 old paths and their stems in file-name context over `*.md` (excluding `PLAN_*.md`, `panther/`), `lib/`, `server/`, `client/src/`; no em-dash added. Floor green at HEAD: `deno task typecheck`, `deno task test` (397 passed), `./validate_protocols`, `cd client && npm run build`. |
| 2026-09-22 | 10 | Deviation, prose outside the surface: every doc that described `deno task typecheck` as "server + client + lint:systems" now names `lint:structure` too (`CLAUDE.md` Gates, `PROTOCOL_APP_DEVELOPMENT.md:80`, `PROTOCOL_APP_PLANS.md:21`, `README.md:105`, `SYSTEM_15_admin_ops.md:146`), since the task's contract is what this step changes and R15 has prose move with the code. `SYSTEM_14` had no paragraph naming `lint_systems.ts` to sit beside, so the structure lint's paragraph closes its Contract section, where the test glob step 2 added already lives; `SYSTEMS.md`'s "Running the lint" section is "Running the lints" and shows both tasks. |
| 2026-09-22 | 10 | Gates: `deno task typecheck` runs `lint:structure` after `lint:systems` with no scope argument and prints OK, every check 0. G5 over the whole tree: of the 324 `client/src` paths present at the merge `676d9871` and absent now, none is named in any tracked `.md` outside `PLAN_*.md` and `panther/`. No em-dash added. Floor green: `deno task typecheck` (with the chained lint), `deno task test` (397 passed), `./validate_protocols`, `cd client && npm run build`. |
| 2026-09-22 | 10 | Step 10 built. |
| 2026-09-22 | 6 | Finding (Review 10): `client/src/generate_visualization/figure_staleness.ts:8`, the comment names `components/figure_editor/stale_figure_badge.tsx`; step 6 (`982736a2`) moved the folder to `components/_shared/figure_editor/`. Prose about a moved file moves with it (Review 3, finding 1). Rewrite the path. |
| 2026-09-22 | 7 | Finding (Review 10): `client/src/generate_visualization/resolve_figure_from_metric.ts:18`, the comment names `slide_deck/slide_ai`; step 7 (`5701655c`) moved it to `components/products/copilot/slide_ai/`. Rewrite the path. |
| 2026-09-22 | 7 | Finding (Review 10): `client/src/components/products/slide_deck/slide_editor/slide_editor.tsx:248`, the comment names `_shared/cursors/slide_cursors.tsx`; step 7 dissolved `_shared/cursors/` and the file is `slide_cursors.tsx` in the comment's own folder. Rewrite the path. |
| 2026-09-22 | 7 | Finding (Review 10): `client/src/components/data/hfa/indicators/ai/wrapper.tsx:23`, the comment says "fully isolated from components/copilot"; step 7 (`5701655c`) moved the copilot to `components/products/copilot/`. Rewrite the path. |
| 2026-09-22 | 10 | Reviewer's reading, no change needed. Surface: `155303a7` is the step's one commit; `deno.json` changes the `typecheck` task only; `CLAUDE.md` (Gates bullet), `PROTOCOL_APP_DEVELOPMENT.md:80`, `PROTOCOL_APP_PLANS.md:21`, `README.md:105` and `SYSTEM_15_admin_ops.md:146` are outside the step's list, each a rewrite of prose that described what `deno task typecheck` runs, which is the contract this step changes; accepted as Reviews 3 to 8 accepted the prose rewrites R15 requires. The SYSTEM_14 placement is accepted: the file had no paragraph on `lint_systems.ts`, so the new paragraph (lines 108 to 114) closes its Contract section and names both scripts, the self-test and the protocol; `SYSTEMS.md` has no gates list, so "Running the lints" (lines 141 to 162) holds both tasks and a paragraph that names the nine checks in prose and the protocol path. Deliverable read from the code: the `typecheck` task chains `deno task lint:structure` after `deno task lint:systems` with no scope argument. G5 whole tree: 324 `client/src` paths present at `676d9871` and absent now; `git grep -F -f` over `*.md` (excluding `PLAN_*.md`, `panther/`) is empty, and over `lib/`, `server/`, `client/src/` empty for the full paths; every `client/src/...`, `~/...` and bare `components/...` path named in a doc or a code comment exists on disk, except the synthetic tree in `server/tests/lint_structure_test.ts` and the four comments above, which the `.md`-only G5 of §5 does not reach. `instance_population_type_grid` at `state/instance/t2_population.ts:17` is a state key, not a path. Build-log facts hold: 324, the five docs at the lines named, the SYSTEM_14 paragraph under Contract (the test glob is line 24 of the manifest). No em-dash added. No file outside the plan names `PLAN_COMPONENTS_TREE`. Gates: `deno task typecheck` green with `lint:systems` OK (898 tracked files) and `lint:structure` OK, every check 0; `deno task lint:structure` alone the same; `deno task test` 397 passed, 13 tests from `server/tests/lint_structure_test.ts` (G6); `./validate_protocols` passed, 0 violations, 16 baselined; `cd client && npm run build` built. |
| 2026-09-22 | 10 | Step 10 reviewed: 4 findings. |
| 2026-09-22 | 10 | Fix: the four comments name their files' current homes: `generate_visualization/figure_staleness.ts:8` says `components/_shared/figure_editor/stale_figure_badge.tsx`, `generate_visualization/resolve_figure_from_metric.ts:18` says `components/products/copilot/slide_ai`, `products/slide_deck/slide_editor/slide_editor.tsx:248` says `slide_cursors.tsx` (its own folder), `data/hfa/indicators/ai/wrapper.tsx:23` says `components/products/copilot`. A sweep for the four old forms over `client/src/`, `lib/`, `server/` and `*.md` (excluding `PLAN_*.md` and `panther/`) finds nothing else. No em-dash added. Floor green: `deno task typecheck` (with the chained lint), `deno task test` (397 passed), `./validate_protocols`, `cd client && npm run build`. |
| 2026-09-22 | 10 | Step 10 fixed. |
| 2026-09-22 | 10 | Reviewer's reading, no change needed. Re-review after Fix 10, scoped to `git diff 4f0101c0..HEAD`: `5fe63940` touches the four files findings 1 to 4 named and the plan, one comment line each. Each rewritten line names its file's current path, confirmed with `git ls-files`: `components/_shared/figure_editor/stale_figure_badge.tsx` (`generate_visualization/figure_staleness.ts:8`), `components/products/copilot/slide_ai` (`generate_visualization/resolve_figure_from_metric.ts:18`), `slide_cursors.tsx` in the comment's own folder (`products/slide_deck/slide_editor/slide_editor.tsx:248`), `components/products/copilot` (`data/hfa/indicators/ai/wrapper.tsx:23`). The four old forms (`components/figure_editor/`, `slide_deck/slide_ai`, `_shared/cursors`, `components/copilot` not preceded by `products/`) have no hit over `client/src/`, `lib/`, `server/` and `*.md` excluding `PLAN_*.md` and `panther/`. No em-dash added. No file outside the plan names `PLAN_COMPONENTS_TREE`. Floor green at HEAD: `deno task typecheck` with `lint:systems` OK and `lint:structure` OK, every check 0; `deno task test` 397 passed, 13 from `server/tests/lint_structure_test.ts`; `./validate_protocols` passed, 16 baselined; `cd client && npm run build` built. |
| 2026-09-22 | 10 | Step 10 reviewed: pass. |
