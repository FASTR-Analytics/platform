...
> **Status: IMPLEMENTED 2026-06-11** (commits b76c1292 A1 reset, 58451f62 split + weights, e05c2577 wizard shell + ICEH port). Verified: `deno task typecheck` green; `./validate_migrations` passed (fresh-DB replay idempotent); migrations applied cleanly to the real local instance + 6 project DBs; backfill counts equal; FK names + RESTRICT DEFERRABLE preserved; structure/items + replicant-options APIs exercised incl. the facility-join branch against `facilities_hmis`; ICEH wizard verified in browser on the new shell. **Still pending manual verification:** end-to-end structure/HMIS/HFA imports with real CSVs (mutating; not run), removing one dataset family from a both-family project. **Remaining work:** HFA/HMIS/structure wizard ports (PLAN_IMPORTER_CONSOLIDATION M4c), weights ingestion + weighted means (§8).
>
