...
| 2026-09-12 | 3 | Do 3, by a fresh agent. The app tree was clean on `tim-branch`. The site repo (`../wb-fastr-site`, branch `main`) was not: `admin-guide/indicators.md` and its French twin carried an uncommitted A3-era paragraph (population terms written `[population:pregnancies]`, the insert pickers and the legend). Its facts were folded into the rewrite, with the population syntax corrected to the bare type id (`anc4 / population_pregnancies`, migration 084) and the coverage claim corrected to S8 "population.csv" (generation refuses only a type with no rows at all; uncovered cells are absent, not refused). |
| 2026-09-12 | 3 | SYSTEM prose. S5: the HMIS dictionary is described as §2 wherever "common" meant an indicator (the derived-indicators paragraph, the additivity ruling's type list, the presentation-fields bullet, the population-store pointer, the editor palette and picker), the migration paragraph keeps "raw", "common" and "mapping" because they name what 086 reads, and four plain-English uses of "source" are reworded (label authority, two workbook inputs, one home, origin marker). S6: "both routes" / "either route" for the import routes, dataValueSets "authoritative", the Columns step names the indicator column, origin marker. S8: the v6 manifest label reads "the indicator restructure" and `judgeDerivedIndicator` "judges definitions, not data". S7: unchanged (its only hits are ruling 13's `credentialsSource`). |
| 2026-09-12 | 3 | Site (`wb-fastr-site`, one commit on `main`). `admin-guide/indicators.md` and `fr/`: the HMIS section is rewritten as §2 (one list; Type DHIS2 element / Uploaded / Sum / Derived; Import from DHIS2 and the naming step with assignment to an uploaded indicator, "already imported" and decomposition; sums; derived indicators with population terms; Include in analysis; the nine-column batch file and replace), with help ids `ind-list`, `ind-dhis2-import`, `ind-derived`, `ind-include`, `ind-batch` replacing `ind-common` and `ind-calculated` (no app button used either; `ind-r-code` unchanged); the intro and Best practices lines that said "map to raw data", "common indicator mappings" and "calculated indicators" are reworded. `admin-guide/data-hmis.md` and `fr/`: Starting an import (the Imports button and its four tabs, New import, the queue), the CSV workflow (Upload / Columns / Review & launch, `indicator_id` among the four fields, the Needs review hold's three actions, merge semantics), the DHIS2 workflow (Credentials / Indicators / Time / Config / Review & launch, the expansion of sums and derived indicators, per-pair scoped delete-then-insert, skip-and-record, the DHIS2-indicator and unknown-id classes), Validation, and Managing import history (History, By indicator, Re-import this indicator, Retry failed pairs, the delete window over indicators); the "View previous imports" and "abort on any failure" text, which described screens that no longer exist, is gone. Every bold label is the UI's own string in both languages. The Deleting ICEH data and After importing sections and the HFA sections are untouched. An independent review agent read the four sections for em-dashes, flourish, jargon, long sentences and French agreement; its findings were applied except the spaced hyphens, which the site's content guide mandates. `verify-help-tags.ts` (what `pnpm verify:help-tags` runs) passes, 44 targets. |
| 2026-09-12 | 3 | Generated table: `deno task build:help-buttons` run twice; the second run left `git status` unchanged. The build's summary stripper drops `_` and `*` from the first 200 characters of prose (its emphasis regex), so the three sections whose lead sentences named `facility_id`, the `*` operator or `indicator_id` were given a lead sentence that ends before them; `build_help_buttons.ts` is outside the Surface and is not changed. |
| 2026-09-12 | 3 | **Step-3 gate, code wins, recorded as Review 2 recorded step 2's.** `grep -rni "source\|indicator_mappings\|indicators_raw\|is_default" SYSTEM_05 SYSTEM_06 SYSTEM_07` is at zero for `indicator_mappings`, `indicators_raw` and `is_default`, and for "source" outside ruling 13's concepts and three other names: S7 255, 260, 322 (`credentialsSource` / `Dhis2RunCredentialsSource`, ruling 13 concept 1); S6 84, 87, 89 (the `source` column of `dataset_hmis_import_runs` and its progress union, concept 2); S5 86-87 (the structure wizard's step 0, the stored `structure_uploads.source_type` column, a rename with a migration for step 4's inventory); S5 391-392 (`fastr-resource-hub`, a repository name, matched through "resource"); S5 603 (the visualization conditional-formatting `indicator` source, S10's concept, `offerIndicatorSource`, for step 4's inventory); S5 742, 746 (`source_name` and `__source__`, stored geojson match keys, for step 4's inventory). S8:1078 keeps "calculated-indicators generator" in an open item that explains why R-source interpolation needs hardening. |
| 2026-09-12 | 3 | Gates run: `deno task typecheck` exit 0 (server, client, `lint:systems`, rerun after the final help-table build); `deno task test` (93 passed, 0 failed); `./validate_protocols` (0 tier-1, 0 new tier-2, 17 baselined); `deno task build:help-buttons` twice with the tree unchanged after the second; the step-3 grep as the row above. The boot: port 8000 was held by another deno process (pid 50020, not this session's), so the server was started the way `./run` starts it with `PORT=8001` (the PLAN_A3 precedent), Valkey up in `valkey-local`; it applied no migration (the JSON sweeps and 13 manifests checked, 0 transformed), listened after 8 s, answered `GET /` with 200, and was stopped; port 8001 is free and port 8000 was not touched. |
| 2026-09-12 | 3 | Step 3 built. |
| 2026-09-12 | 3 | Review 3, by a fresh agent. Tree clean on `tim-branch`; the site repo clean on `main`. Step 3 is one app commit (`da323051`) and one site commit (`34d0194`). Surface: the app commit touches SYSTEM_05, SYSTEM_06, SYSTEM_08, the generated help table and this file; the site commit touches the two admin-guide pages and their French twins. Nothing outside the Surface. |
| 2026-09-12 | 3 | Deliverable, established by reading. SYSTEM_05, 06 and 08 describe the dictionary as §2: the remaining "raw", "common" and "mapping" words are the 086 migration paragraph (S5 345-370, what the migration reads), the two stamp names ruling 13 lists for step 4 (S5 827-832), HFA and CSV column-mapping prose that is not the HMIS dictionary, and S8 1078's open item. The site pages name indicators, DHIS2 elements, uploaded indicators, sums, derived indicators and Include in analysis; every bold label was grepped in `client/src` and `lib` and each is a UI string; the id validator, the three formats, the special-id rule, the sum-member rule, the schedule's stored-credentials rule (`datasets.ts` 273), the delete-while-running refusal (`dataset_hmis.ts` 117) and the expansion's dropped uploaded indicators (`dataset_hmis_import_runs.ts` 229) all read as the pages say. Help tags: en and fr carry the same six `ind-*` and three `hmis-*` tags; `pnpm verify:help-tags` passes; the client's only `HelpButton` is `viz-data-tab`, and `HelpId` is a generated union, so the retired ids could not have survived the typecheck. |
| 2026-09-12 | 3 | Gates run by the reviewer: `deno task typecheck` exit 0 with `lint:systems` OK; `deno task test` 93 passed, 0 failed; `./validate_protocols` 0 tier-1, 0 new tier-2, 17 baselined; `deno task build:help-buttons` twice, 44 targets, `git status` clean after both; the step-3 grep gives exactly the hit list of the Do 3 row (S7 255/260/322, S6 84/87/89, S5 86-87/391-392/603/742/746), all ruling 13 concepts, a repository name, or stored keys outside the dictionary. Boot: port 8000 still held by pid 50020, so the server was started as `./run` starts it with `PORT=8001`, Valkey already up; 13 manifests and every project checked with 0 transformed, listening after 8 s, `GET /` 200, stopped, port released. |
| 2026-09-12 | 3 | Observation, not a finding (outside the Surface): the HFA sections of the site's `admin-guide/indicators.md` and its French twin still carry em-dashes (en lines 109 and 115, fr 109 and 115), from before this plan. Step 4's site pass or a later doc session can take them. |
| 2026-09-12 | 3 | Step 3 reviewed: pass. |
| 2026-09-12 | 4 | Do 4, by a fresh agent. Tree clean on `tim-branch`. **Inventory**, the step's work list: `grep -rniw "source\|sources\|common\|mapping\|mappings" lib server client/src SYSTEM_*.md PROTOCOL_APP_*.md` outside migrations gives 703 hits in 205 files, classed as follows. **Rename, the removed entity's word:** "common indicator(s)" and "commons" in comments, prose and strings (lib `types/indicators.ts`, `types/population.ts`, `types/conditional_formatting*.ts`, `resolve_effective_indicator_facts.ts`, `indicator_expression/{parse,resolve}.ts` including the user strings at resolve 100, 153 and 158, `ai_tools/build_system_prompt.ts`, `types/run_manifest.ts` 25/30/178; server `db/project/datasets_in_project_hmis.ts` 99-100, `runs/{indicator_catalog,manifest_transform}.ts`, `routes/caches/visualizations.ts` 72, `run_query/catalog_expression_items.ts`, `worker_routines/generate_run/prepare_inputs.ts`, `dhis2/goal2_indicators/get_indicators_from_dhis2.ts` 9, the `common(...)` fixture helper of `m012_expression_parity_test.ts`; S2 288, S5 (the four family hits), S8 (two), S9 99, S10 432, S13 713, PROTOCOL_APP_STATE 249). **Rename, ruling 13's concepts:** (1) `Dhis2RunCredentialsSource` / `credentialsSource` (20 files, S7 255/260/322, PROTOCOL_APP_WORKER_ROUTINES 191); (2) the import route: the `source` column of `dataset_hmis_import_runs` and `dataset_hmis_import_ledger` (`_main_database.sql` 450/470, `_main_database_types.ts` 130), the `source` field of the run summary, the ledger item and the queued-run union, `stagingResult.sourceType` on version rows and inside `run_stats.csvStagingResult`, S6 84-89; (3) the CSV columns: `HmisCsvMappingParams`, the `mappings` key of `csv_config` (`_main_database.sql` 464, `lib/api-routes/instance/datasets.ts` 105-110), the two "column mappings" worker strings, S6 86; (4) the dictionary stamps `indicatorMappingsVersion` / `baseIndicatorMappingsVersion`, their getters, the project `datasets.info` JSON that stores them, PROTOCOL_APP_STATE 102/245/249, S5 827-832/870, S6 426; (5) the type family `CommonIndicator`, `CommonIndicatorDefinition`, `CommonIndicatorType`, `COMMON_INDICATOR_TYPES`, `isCommonIndicatorType`, `getCommonIndicators`, `buildCommonIndicatorDictionary`, `resolveCommonIndicatorCatalog`, `CommonIndicatorCatalogRow`, `CommonIndicatorCatalogError`, `lib/common_indicator_catalog.ts`, `RunCommonIndicator`, `runCommonIndicatorSchema`, `buildRunCommonIndicators`, and the key `commonIndicators` on the run manifest, the project SSE state and the AI grounding. **Keep, `indicator_common_id`:** every `common.indicator_common_id` and the column. **Keep, a field name owned by another repo or an external format:** the module definition's `dataSources[].sourceType` (`wb-fastr-modules` definition.json: `lib/types/_module_definition_*.ts`, `lib/dataset_family.ts`, `generate_run/{resolve_modules,resolve_reuse,prepare_inputs}.ts`, `runs/generation_wizard_reads.ts`, `get_script_with_parameters.ts`, S8 987); the metric disaggregation option `source_indicator` (`lib/disaggregation_labels.ts`); the ICEH Retriever export's `Source` column (`iceh_data.source`: `import_iceh_data/ingest.ts`, `db/instance/dataset_iceh.ts`, `db/project/datasets_in_project_iceh.ts`, `lib/types/dataset_iceh.ts`, `_main_database.sql` 787/797); the Anthropic document block's `source` (`useAIDocuments.ts`); Tailwind's `@source` and CSS `no-common-ligatures` (`app.css`). **Keep, an English word that is not a name:** "single source of truth", "source of truth", "the common case", "common functions", "Common Table Expression", "source line", "source data row", "source deck/project/version/run", "sources credentials" (a verb, `lib/types/dhis2.ts` 3), and "mapping" as a noun in the auth, version-diff, cursor, rebase and scroll-sync comments (S1 326-329, S16 545/1116). **Keep, code wins, a name for another concept outside the HMIS dictionary and its imports (recorded for Tim to rule on in Review 4):** the structure wizard's `source_type` column, `sourceType` and step-0 "choose source", its `Mappings` and `IndicatorSubMappings` kernel types and step-2 column mappings (S0 49, S5 86-87/184-187, `_main_database.sql` 389, `lib/types/instance.ts` 426, `lib/types/structure.ts`, `structure_import/**`, `db/instance/structure.ts`, `server_only_funcs_importing/**`); the HFA CSV import wizard's `HfaCsvMappingParams`, its `csv_config.mappings` and "Mappings" step (S6 207-218/302/414, `_main_database.sql` 618, `lib/types/dataset_hfa_import.ts`, `dataset_hfa_import_runs.ts`, `import_hfa_data_csv/**`, `instance_dataset_hfa/imports/_wizard.tsx`), the HFA workbook `HfaWorkbookSource` and time-point `mapping`, the HFA weights `Mappings`; the geojson wizard's `source` (file or DHIS2), its area `mapping` and the stored match keys `source_name` / `__source__` (S5 756-766, S7 206, `instance_geojson/**`, `routes/instance/geojson_maps.ts`, `dhis2/goal4_geojson/**`); the DHIS2 connector's `common/` module (S7 37/69/92/96, every `../common/` import); the conditional-formatting `source` and `offerIndicatorSource` (S10's four CF sources: S5 603, `lib/types/indicators.ts` 178/516, `lib/types/conditional_formatting*.ts` 25-50, `lib/resolve_effective_indicator_facts.ts` 14-36/146, `generate_visualization/**`, the CF editors, `get_style_from_po/_0_common.ts`); batch edit-common-properties (S11 235/361, `edit_common_properties_modal.tsx`, `PresentationObjectPanelDisplay.tsx` 974); results-package reuse "source" (S8 733/1107, `resolve_reuse.ts`, `execute_module.ts`); the expression tokenizers' `source` parameter (`parse.ts`, `resolve.ts` 67-168, `decompose_indicator.ts`) and `renameIdentifiers`' `mapping`; the MCP `Source:` header (S13 114, `mcp/context_cache.ts` 202, `mcp_tools_source_header_test.ts`); `lib/translate/common.ts` (S14 182); the generic CSV `mappings` helper (`get_csv_components_streaming_fast.ts` 424); local names in `hfa_dependency_analyzer.ts`, `hfa_indicators.ts` 524, `versions.ts` 312, `projects.ts` 901, `apply_figure_config_patch.ts`, `validate_figure_config_edit.ts`, `describe_figure_config_patch_effect.ts`; the AI tool descriptions in `slides.tsx` and `drafts.tsx`; the onboarding tour string; S15 95/152/190/318-324 and PROTOCOL_APP_UI_CONVENTIONS 215-220. Reason: ruling 13 defines its class as names that survived step 2 and step 3's gates, and every one of those is in the HMIS dictionary or its imports; the names in this class were in no gate, belong to other systems, and three are stored columns or keys of their own whose rename is a migration for a word that names nothing this plan removed. |
| 2026-09-12 | 4 | Concept 1, the credentials origin, one commit: `Dhis2RunCredentialsSource` is `Dhis2CredentialsOrigin`, the route body and worker-message key `credentialsSource` is `credentialsOrigin` (same length, so the PROTOCOL_APP_WORKER_ROUTINES table keeps its columns), the two route schemas `dhis2RunCredentialsSourceSchema` are `dhis2CredentialsOriginSchema`, and the geojson wizard's signal follows. Nothing stores it: the scheduler builds `{ kind: "stored" }` at fire time and the worker message is transient. S7 255/260/322 read the new names. |
| 2026-09-12 | 4 | Concept 2, the import route, one commit. The `source` column of `dataset_hmis_import_runs` and `dataset_hmis_import_ledger` is `route` (with its CHECK constraint name), the run summary, the ledger item, the queued-run union and the DB row type carry `route`, and the staging result's discriminator `sourceType` (csv, dhis2, deletion; on version rows and inside a CSV run's `run_stats.csvStagingResult`) is `kind`. Migration `087_import_route.sql` does the guarded column and constraint renames and the two JSON rewrites; the JSON is read by cast (`parseJsonOrThrow`), not by a Zod sweep, so there is no skip-gate to force: the SQL rewrite is the whole transform. Two older migrations needed the A3 treatment for the fresh replay: 070's `ADD COLUMN source` is guarded on `route` not existing, and 086's two `run_stats` updates select by the blob's shape (`csvStagingResult` present or not) instead of the column. `validate_indicator_migration.ts` (outside the listed Surface, forced: it scans post-migration rows) reads `kind`. S6 84-89 and the schema comments read `route`. |
| 2026-09-12 | 4 | Concept 3, the CSV columns, one commit. `HmisCsvMappingParams` is `HmisCsvColumns`; the launch input, the stored run config, the route body and the stage-leg argument carry `columns`; the two worker strings read "Check the columns and try again" and the launch check "No column chosen for …". Migration `088_csv_config_columns.sql` renames the `mappings` key of `csv_config` (read by cast, so the rewrite is the whole transform; 086's own `csv_config -> 'mappings'` rewrite runs before it). The HFA wizard's `HfaCsvMappingParams` and its `csv_config.mappings` are the inventory's keep class and are untouched: the closest call in that class, recorded for Tim. S6 86 and the schema comment read `columns`. |
| 2026-09-12 | 4 | Concept 4, the dictionary stamps, one commit. `indicatorMappingsVersion` is `indicatorsVersion` and `baseIndicatorMappingsVersion` is `baseIndicatorsVersion`, with the getters `getIndicatorsVersion` and `getBaseIndicatorsVersion`, across the instance summary, the SSE payload, the datasets route body, the client stores and the captured `DatasetHmisInfoInProject`. The stamps are stored in the project database's `datasets.info` JSON at capture, so project migration `042_dataset_info_stamp_names.sql` renames the two keys there (read by cast; nothing compares the stored pair today, the rename keeps the stored shape equal to the type). The run manifest's `datasets[].info` is `z.unknown()` provenance captured at finalize and is left as written: a manifest block may not rewrite provenance, and nothing reads those keys. `instance_indicators_v5` keeps its name: the payload shape is unchanged, only the parameter name moved. S5 827-832 no longer says the names are kept; S5 870, S6 426 and PROTOCOL_APP_STATE 102/245/249 read the new names. |
| 2026-09-12 | 4 | Concept 5, the indicator type family, one commit. `CommonIndicator` is `HmisIndicator` (with `HmisIndicatorDefinition`, `HmisIndicatorType`, `HMIS_INDICATOR_TYPES`, `isHmisIndicatorType`, `HmisIndicatorCatalogRow`, `HmisIndicatorCatalogError`, `getHmisIndicators`, `buildHmisIndicatorDictionary`, `resolveHmisIndicatorCatalog`, `RunHmisIndicator`, `runHmisIndicatorSchema`, `buildRunHmisIndicators`, `INDICATOR_COLUMNS`); `lib/common_indicator_catalog.ts` is `lib/hmis_indicator_catalog.ts` (S5 glob, `lib/mod.ts`, S5 280/519, S8 911); the run manifest, the project detail, the project SSE state and the AI grounding carry `hmisIndicators`. The manifest key is a stored key, so `RUN_MANIFEST_SCHEMA_VERSION` is 8 and manifest transform block 6 drops the legacy `commonIndicators` key (block 4 already stamps the list under the new name on every forced pass); every package on disk is rewritten once at the next boot, with the usual `manifest.v7.json` copy beside it. "HMIS" is the qualifier because HFA and ICEH indicators share the codebase; `hmisIndicators` also names a count in the instance summary, a different object. Comments and prose say HMIS indicator, indicator, base or ingredient; the three user strings of `resolve.ts` (100, 153, 158) and the AI prompt heading read "indicator", "at least one indicator", "ingredient indicators" and "HMIS indicators". The m012 fixture changed only in its helper and constant names (`indicator(...)`, `INDICATORS`); the data is the same. `validate_indicator_migration.ts` (outside the listed Surface, forced by the type import) reads `HmisIndicator`; its own pre-086 vocabulary (`commons`, `mappingsByRaw`) describes the tables it reads before the migration and stays. S2 288, S9 99, S10 403/432, PROTOCOL_APP_STATE 145 renamed; S13 713 ("common dimensions") and S5 370 (the 086 paragraph) are keep-class. |
| 2026-09-12 | 4 | Concept 6, the doc survivors, one commit: the last three rename-class lines the concept commits did not reach. `lib/types/indicators.ts` 521 ("Common-indicator evaluation" is "Expression evaluation"), `scheduler.ts` 383 (the fire path expands "to DHIS2 elements", not "to sources"), and S15 190 (the health export route described as what it returns: every DHIS2 element with the indicator that carries it, under the wire keys the Admin-Website reads). The `describeDhis2Source*` and `hmisSources`-era words ruling 13 anticipated were already gone: step 2 renamed the former and step 3 the latter. |
| 2026-09-12 | 4 | Concept 1 leftover, one commit: `resolveDhis2Credentials`' parameter was still named `source` (`instance_dhis2_credentials.ts` 159); it is `origin`. Found by the closing inventory grep, which is the reason the grep is the gate and not the rename list. |
| 2026-09-12 | 4 | Three stored homes moved, three migrations: instance `087_import_route.sql` (columns and constraint names, the staging `kind` key), instance `088_csv_config_columns.sql` (the `columns` key) and project `042_dataset_info_stamp_names.sql` (the two stamp keys). The plan's Surface names `server/db/migrations/instance/` only; the stamps are stored in the project database, so 042 is in `server/db/migrations/project/`, outside the listed Surface and forced by where the key lives. The manifest key moved with `RUN_MANIFEST_SCHEMA_VERSION` 8 and block 6. No Valkey prefix moved: no cached payload carries a renamed key (the project detail and the AI grounding are built live from the manifest cache, and the datatable and dictionary payloads keep their shapes). The dev `main` database was migrated by a boot of this tree at 14:29 (pid 22772, started outside this session, the way `./run` starts it): it is at `088_csv_config_columns`, the 11 run rows read `route = dhis2`, 36 of 37 version rows carry `kind` (the 37th never had `sourceType`), no `csv_config` carries `mappings`, all 12 project databases are at `042_dataset_info_stamp_names`, and the 13 packages in `_example_instance_dir/runs` are at manifest v8 with `hmisIndicators` and a `manifest.v7.json` beside each. |
| 2026-09-12 | 4 | Gates on the final tree: `deno task typecheck` exit 0 (server, client, `lint:systems` OK); `deno task test` 93 passed, 0 failed (run alone and again by the dev boot); `./validate_protocols` (0 tier-1, 0 new tier-2, 17 baselined); `./validate_migrations` (91 instance and 44 project migrations, schema unchanged on the fresh replay; the run that failed before 070 was guarded is the reason 070 is guarded); `./validate_queries` (63 cases); `./validate_fresh_boot` (22 specials as empty bases, every checkbox on); `./validate_indicator_migration ~/wb-fastr-dev-main-before-086.sql.gz` PASS through 086, 087 and 088 (15 raws: 7 folded, 8 new bases, 2 sums, 1 derived special renamed; 371,087 data rows, 2,085 ledger rows, 11 run rows, 37 version rows). The inventory grep is at 557 hits in 174 files, every one in a keep class of the inventory row (the rename-class words `credentialsSource`, `Dhis2RunCredentialsSource`, `HmisCsvMappingParams`, `MappingsVersion`, `CommonIndicator`, `commonIndicators`, "common indicator", the HMIS `source` column and `sourceType` are at zero outside `server/db/migrations/**` and the legacy-key comments of the manifest transform). The step-2 grep over the three client surfaces is at zero with no exception. Gate 2 word-boundary at zero outside migrations except the `is_default` comment gate 2 excludes; gate 1 at zero. The boot: port 8000 was held by pid 22772, so the server was started the way `./run` starts it with `PORT=8001` (the PLAN_A3 precedent), Valkey up in `valkey-local`; it applied no migration (13 projects and 13 manifests checked, 0 transformed), ran the 93 tests, listened, answered `GET /` with 200, and was stopped; port 8001 is free and port 8000 was not touched. |
| 2026-09-12 | 4 | Step 4 built. Next step `Review 4`. The reviewer's first ruling is the inventory row's keep class "a name for another concept outside the HMIS dictionary and its imports": accepting it closes the plan; rejecting any part of it is a Fix 4 item naming the concept. |
| 2026-09-12 | 4 | Review 4, by a fresh agent that did not write the code. Tree clean on `tim-branch`; the step's commits are 8ed57a9c..48696421 (nine after 27deeb85). Every gate run by the reviewer on the final tree: `deno task typecheck` exit 0; `deno task test` 93 passed, 0 failed; `./validate_protocols` 0 tier-1, 0 new tier-2, 17 baselined; `./validate_migrations` 91 instance and 44 project migrations, schema unchanged; `./validate_queries` 63 cases; `./validate_fresh_boot` 22 specials as empty bases, every checkbox on; `./validate_indicator_migration ~/wb-fastr-dev-main-before-086.sql.gz` PASS through 086, 087 and 088 (15 raws: 7 folded, 8 new bases, 2 sums, 1 derived special renamed); the boot on `PORT=8001` against the dev database (port 8000 held by pid 22772, untouched) applied no migration, ran the 93 tests, answered `GET /` 200 and was stopped. The inventory grep is at 554 hits in 173 files, every one read and in a keep class of the inventory row; the rename-class words are at zero outside `server/db/migrations/**` and the manifest transform's legacy-key comments; the step-2 grep over the three client surfaces is at zero; gate 1 at zero; gate 2's seven hits (`resource_id`, `offerIndicatorSource`, the `is_default` comment) are substrings of other names, the same seven present at 27deeb85. The diff was read whole (104 files): every rename is consistent across the three tiers, no runtime read or write of an old name survives (SQL column references, stored JSON keys, route bodies, worker messages and client reads checked), the m012 fixture changed only in helper and constant names, and 087, 088, project 042 and manifest block 6 are guarded and idempotent. **Ruling on the keep class** "a name for another concept outside the HMIS dictionary and its imports": accepted. Ruling 13's class is the survivors of step 2 and step 3, all in the HMIS dictionary and its imports; the structure wizard's `source_type`, the HFA wizard's `csv_config.mappings`, the geojson wizard's `source` and the rest of that class name other systems' concepts, three of them stored keys whose rename would be a migration for a word this plan did not remove. Tim can reopen any of them as its own item. |
| 2026-09-12 | 4 | Finding: `validate_indicator_migration.ts` 30. The concept-5 rename reached the retired-key list: `"hmisIndicatorsToInclude"` names a key that never existed, while the key 086 strips from windowing JSON is `commonIndicatorsToInclude` (086 lines 279-283). `retiredKeysIn` matches by exact string, so a leftover `commonIndicatorsToInclude` in a scanned column would now pass silently. Restore the string; the validator's pre-086 vocabulary is keep-class, as the concept-5 row already says. |
| 2026-09-12 | 4 | Finding: `SYSTEM_09_viz_query_cache.md` 104. The sweep renamed the file reference `PLAN_1_COMMON_INDICATOR_TYPES.md` to `PLAN_1_HMIS_INDICATOR_TYPES.md`; no file of the new name has ever existed. The old name is the plan's historical name (deleted at 4dbd986c, in git history), and `grep -w` does not match `COMMON` inside it, so restoring it keeps the gate at zero. Restore it with "in git history", or drop the pointer. |
