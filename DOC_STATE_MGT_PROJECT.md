...
| Server notifications | `server/task_management/notify_project_v2.ts` |
...
| --- | --- | --- | --- |
| Project config | `id`, `label`, `isLocked`, `isCentralReporting`, `aiContext` | `project_config_updated` | — |
...
| Report folders | `reportFolders` | `report_folders_updated` | — |
...
**`aiContext` is in T1 and updated via `project_config_updated`:** The project's AI context string lives in `ProjectState.aiContext`. It is updated whenever `updateProject` is called (same event that carries `label` and `isLocked`). Only the `updateProject` route sends `aiContext` in the event; routes that only change `isLocked` or `isCentralReporting` omit it (the field is optional in the payload).
