...
// =============================================================================
// PRE-DEPLOY FLEET DRY-RUN: project to product consolidation
// =============================================================================
//
// The gate for the products restructure (PLAN_PRODUCTS_RESTRUCTURE D13). Run
...
// server/db/migrations/consolidation/plan.ts is exactly what 201 executes, and
...
// touches the instance containers. Read-only throughout (PROTOCOL_ACCESS_DBS
// "Safety").
//
...
//           that are not migrated (D11). None of these stop the migration; all
//           of them are irreversible once it runs.
//
// Usage:
...
//                                                     per-instance counts
//
// The `--json` file (default rollout_logs/consolidation_plan.json) is what the
// step 10 post-check compares each migrated instance against, so the numbers
// the deploy is verified against are the numbers this gate approved.
//
...
// (PROTOCOL_ACCESS_DBS recipe B), then tears it down. Nothing is written to
// the server and no password reaches argv.
//
// =============================================================================
...
import {
  ...
  projectDatabaseExists,
  readPinnedRunId,
  readProjects,
  REQUIRED_SOURCE_MIGRATION,
  seedTakenIds,
} from "./server/db/migrations/consolidation/execute.ts";
...
export type InstanceReport = {
  ...
  dbAbsent: ProjectRef[];
  notAtRequiredMigration: ProjectRef[];
...
// `docker port` prints one line per bound address; every line carries the
// same host port, so the first is enough.
...
async function resolvePgPassword(instance: string): Promise<string> {
  return await ssh(
    `docker exec ${instance}-postgres printenv POSTGRES_PASSWORD`,
  );
}
...
// A direct pool rather than the app's connection factories: the target is a
// tunnelled fleet instance, not the PG_* environment.
...
export async function dryRunInstance(instance: Instance): Promise<InstanceReport> {
  ...
  try {
    // The pin (D5): the run_id for a project that has none.
    report.pinnedRunId = await readPinnedRunId(mainDb);
    if (report.pinnedRunId !== null) {
      const pinned = await mainDb<{ label: string }[]>`
        SELECT label FROM runs WHERE id = ${report.pinnedRunId}
      `;
      report.pinnedRunLabel = pinned[0]?.label ?? null;
    }

    // Projects by status and the two D11 lists. Ordered by id, as 201 orders
    // them, so the ids that collide, and how many, are the migration's own.
    // The replacements are minted at random when 201 runs, so they are not
    // reported.
    ...
    for (const project of projects) {
      ...
      if (!(await projectDatabaseExists(mainDb, project.id))) {
...
// public (dashboards are the app's only unauthenticated surface).
...
type DroppedTotals = ConsolidationPlan["droppedCounts"];
...
export function plannedCounts(report: InstanceReport): PlannedCounts {
  return {
    products: countBy(report, (plan) => plan.products.length),
    ...
    slideDeckVersions: countBy(report, (plan) => plan.slideDeckVersions.length),
    reportVersions: countBy(report, (plan) => plan.reportVersions.length),
    remaps: countBy(report, (plan) => plan.remaps.length),
  };
}
...
function printInstanceReport(report: InstanceReport): void {
  ...
  const counts = plannedCounts(report);
  console.log(
    `  plan:            ${counts.products} products (${counts.slideDecks} decks / ${counts.reports} reports), ` +
      `${counts.folders} folders, ${counts.slides} slides, ` +
      `${counts.slideDeckVersions + counts.reportVersions} versions`,
  );
  console.log(`  id remaps:       ${counts.remaps}`);
  for (const plan of report.plans) {
    for (const remap of plan.remaps) {
      console.log(`      ${remap.entity} ${remap.from} (re-minted by 201)`);
    }
  }
  ...
  if (orphans.length > 0) {
    console.log(`  FK orphans: ${orphans.length}`);
...
function printFleetSummary(reports: InstanceReport[]): boolean {
  ...
  console.log("");
  console.log(
    "  " + "instance".padEnd(20) + "products".padStart(10) +
      "folders".padStart(9) + "slides".padStart(8) + "remaps".padStart(8) +
      "viz-del".padStart(9) + "dash-del".padStart(10) + "  verdict",
  );
  for (const report of reports) {
    ...
    console.log(
      "  " +
        ...
        String(counts.products).padStart(10) +
        String(counts.folders).padStart(9) +
        String(counts.slides).padStart(8) +
        String(counts.remaps).padStart(8) +
  ...
  const maxProducts = reports.reduce(
    (max, report) => Math.max(max, plannedCounts(report).products),
    0,
  );
  const totals = reports.reduce(
    (acc, report) => {
      const dropped = sumDropped(report);
      acc.products += plannedCounts(report).products;
  ...
  console.log("");
  console.log(`  Products fleet-wide: ${totals.products}`);
  console.log(
  ...
  console.log(`    pending_deletion, not migrated:  ${totals.pending}`);
...
// ── The plan file the step 10 post-check compares against ────────────────────

// Defaults into rollout_logs/, which is gitignored, so no untracked file
...
async function writePlanFile(
  ...
  const payload: Record<string, PlannedCounts> = {};
  for (const report of reports) {
    if (report.fails.length === 0) {
      payload[report.instance] = plannedCounts(report);
    }
  }
  ...
  console.log("");
  console.log(`Planned counts written to ${path}.`);
}
...
async function main(): Promise<void> {
  ...
  if (local) {
    ...
    const report = await dryRunInstance(instance);
    ...
    for (const name of instances) {
      try {
        const report = await withTunnel(name, dryRunInstance);
        printInstanceReport(report);
        reports.push(report);
      } catch (error) {
        unreachable++;
  ...
  if (unreachable > 0) {
    console.log("");
    console.log(
      `NOTE: ${unreachable} instance(s) could not be checked. An instance that is not`,
    );
    console.log(
