...
> This doc owns the **worker lifecycle** (spawn → handshake → run → teardown). The dirty-state semantics and the running-tasks-map cleanup *invariant* are owned by [DOC_TASK_EXECUTION_DIRTY_STATE.md](DOC_TASK_EXECUTION_DIRTY_STATE.md) (this doc cites it). Worker DB connections are [DOC_DB_ACCESS_LAYER.md](DOC_DB_ACCESS_LAYER.md). What the dataset workers actually *do* (stage→integrate, bulk insert) is [SYSTEM_06_ingestion.md](SYSTEM_06_ingestion.md); what the module worker does is [DOC_MODULE_EXECUTION.md](DOC_MODULE_EXECUTION.md). Workers reach the main thread's SSE via [DOC_SSE_REALTIME.md](DOC_SSE_REALTIME.md).
