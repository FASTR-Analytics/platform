...
// - get_viz_editor: Get current configuration, available options, valid display modes, and underlying CSV data. Call this first!
// - update_viz_config: Update any aspect of the configuration (type, disaggregations, filters, period, captions, etc.). Changes are LOCAL until user clicks "Save".

// IMPORTANT NOTES:
// 1. Changes you make are LOCAL - the user must click "Save" in the toolbar to persist them
// 2. Always call get_viz_editor first to see current state, available options, and actual data
// 3. Valid display options differ by presentation type (timeseries/table/chart) - check get_viz_editor output
...
// WORKFLOW:
// 1. When user asks about the visualization, call get_viz_editor to see config and data
// 2. Analyze the data and current settings
// 3. If user wants changes, use update_viz_config (only use valid display options from get_viz_editor)
