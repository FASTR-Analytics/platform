...
**Next step: Review 4.** Values are `Do N`, `Review N`, `Fix N`. After step 5's
...
| Step 4 fixed | All ten findings in the `Fix 4` list and all five Deliverable misses worked, each re-verified in the code by this session before the edit rather than taken from the review row. The four errors of fact go back to what the code says: `client/public/fonts/` holds 105 entries of which 102 are fonts, and the sentence now names all three non-font files; `lib/key_colors.ts` has 15 TypeScript consumers, the seventeenth grep hit being the file itself and the sixteenth a SQL migration naming four colours in a literal; `indicator_schema_test.ts` has eighteen constraint and rule cases plus a teardown block; the copilot host wraps whichever editor the Products page opens, one per open product, which is what `products/index.tsx` lines 86 to 97 and `copilot/index.tsx` lines 52 to 54 both say. The over-broad and narrowed claims are re-scoped: `getRunAuthoringContext` now sits beside the four scoped reads under the same guard but with no scope and no ready gate (SYSTEM_09, and the same correction in SYSTEM_08, where the manifest lens was said to share one exposure with `getRunModuleWithConfigSelections`); the proxy anchor is `main.ts:237-238`, line 239 being the Files proxy the same file documents separately; the read-projection formatters name both `lib/ai_tools/` and the copilot's `ai_tools/tools/_internal/`, both live; the SYSTEM_09 cache pointer now makes `server/routes/caches/visualizations.ts` the authority and says plainly that SYSTEM_03's catalog numbers are stale, rather than deferring to a table that still reads 19 and `po_detail_v10`. The barrel paragraph in SYSTEM_02 now describes what the barrels actually do, with the gap (`db/products/mod.ts` absent from `db/mod.ts`, three more siblings missing) stated and added to that file's Open items. The remaining Deliverable misses: SYSTEM_01's zero-perm bullet names `sendHelpEmail` and `recordTourEvent` instead of the send-email route that is now guarded by `requireApprovedUser()`; SYSTEM_09's display-side override reads the bundle's stored scope, not the deleted `projectState` global; SYSTEM_12's two slide-body sentences say both writers take `slideConfigSchema` and that the PatternType none gap is closed; SYSTEM_14's `t3` surface reads 241 in all three places. Gates: `deno task typecheck` PASS (server, client, `lint:systems`), `deno task test` 165 passed 0 failed, `./validate_migrations` PASS, `./validate_migrations_replay` PASS, `./validate_consolidation_replay` PASS, `./validate_fresh_boot` PASS, `./validate_queries` PASS, `./validate_protocols` PASS, `deno task build:help-buttons` leaves `git status` clean. No em-dash added. |
| Step 4 fixed, ruling on the `a27fcaac` findings | Two were raised against that commit rather than against step 4. The prose one is fixed here because it lives in a SYSTEM file this step owns: SYSTEM_08's claim that `PO_CACHE_VERSION` did not move now says it did not move for that change and has moved since. The formatting one is not a defect and is not fixed: the closing angle bracket of `_PO_DETAIL_CACHE`'s type-argument list sits ahead of the prefix comment because `deno fmt` puts it there. Moving it back and rerunning `deno fmt` returns it to the same place, so the shape is the formatter's output for a comment of this length, not a hand edit, and `deno fmt --check` is clean either way. The reviewer's narrowing was right that only `_PO_DETAIL_CACHE` is affected. |
| Step 4 fixed, prose left alone | The review's third row, the pre-existing errors in sentences the merge did not change, is deliberately not worked: the Deliverable scopes this step to contracts the merge changed, correcting the rest would widen a prose-only step into an audit of four systems, and every item there is recorded in that row for whoever owns the file next. The one that most deserves a home is SYSTEM_09's four `btrim` sentences, which name a function the code comment at `query_helpers.ts` lines 31 to 34 explicitly rules out because DuckDB has no `btrim`. |
| Review 4, second pass | Commit `17d51c1d` read against the `Fix 4` list and the five Deliverable misses, by a fresh agent, each item re-checked in the code rather than taken from the commit's own row. Surface held: nine `SYSTEM_*.md` files and this plan's next-step line plus three rows, no code. Thirteen of the fifteen items reproduce and now read as the code does: `client/public/fonts/` is 105 entries, 102 fonts plus the three files the sentence now names; the grep over every symbol `lib/key_colors.ts` exports hits 17 files, less the file itself and `079_common_indicator_types.sql`, so 15 TypeScript consumers; `indicator_schema_test.ts` holds 19 `Deno.test` blocks of which `:271` is the teardown, so eighteen cases; `ProductCopilotHost` has one mount site, the product opener (`client/src/components/products/index.tsx:88-96`, `client/src/components/copilot/index.tsx:52-58`), and `SYSTEM_13:264`'s "the Products page ... have no copilot" now reads consistently beside it; `main.ts:237` and `:238` are the two proxies with `:239` the Files route; `getRunAuthoringContext` is `requireApprovedUser()` with no scope and no ready gate (`server/routes/instance/run_generation.ts:335-341`) against the four `getReadyRunReadContext` reads, corrected the same way in SYSTEM_08 and SYSTEM_09, and the 95-character line is reflowed (nothing over 80 in the edited regions); both formatter sets are live (`lib/ai_tools/format_metric*_for_ai.ts` and the copilot's `_internal/format_figure_config|module_settings|modules_list_for_ai.ts`); the cache pointer matches `server/routes/caches/visualizations.ts:95,147` ("23", `po_detail_v13`) and `SYSTEM_03:369-374` is indeed still "19" and `po_detail_v10`; `t3(` is called in 241 client files and SYSTEM_14 says 241 in all three places; `projectState` has zero hits under `client/` and `lib/` and the override reads the bundle scope (`get_data_config_from_po.ts:126-132`); `createSlide` and `updateSlide` both take `slide: slideConfigSchema` (`lib/api-routes/products/slides.ts:49,59`) and the split-fill enum mirrors `PatternType` with `"none"` (`lib/types/_slide_config.ts:25-32`); `PO_CACHE_VERSION` is scoped to the D7 change with the later bumps acknowledged. Gates: `deno task typecheck` PASS (server, client, `lint:systems`, every tracked file claimed once), `deno task test` 165 passed 0 failed, `./validate_migrations` PASS, `./validate_migrations_replay` PASS, `./validate_consolidation_replay` PASS, `./validate_fresh_boot` PASS, `./validate_queries` PASS, `./validate_protocols` PASS (0 tier-1, 0 new tier-2, 16 baselined), `deno task build:help-buttons` leaves `git status` clean. No em-dash added by the commit. |
| Review 4, second pass, findings (the Fix 4 list) | Two, both in `17d51c1d`'s own new text, both one line. (1) `SYSTEM_01_api_contract.md:638-640` traded the old over-broad example for a new over-broad claim: "A route that needs the flag takes `requireApprovedUser()` instead, which is where every product and package read now sits." Neither half holds. Product and folder routes take `requireProductAccess(level)`, which reaches the flag through `productAccessPolicy` (`server/middleware/userPermission.ts:134,147`; `server/auth/product_access.ts:16-21`) and never through `requireApprovedUser()`, as the same file says at `:506-511`. The package-internals reads `getRunModuleScript`, `listRunModuleFiles`, `getRunDetail` and `getRunModuleWithConfigSelections` take `requireGlobalPermission("can_view_data")` (`server/routes/instance/run_generation.ts:177,197,210,222`), whose body checks the permission and never `approved` (`userPermission.ts:190-238`), so they are counter-examples to the bullet they are cited under, and the same commit's SYSTEM_08 edit now says so in the other file. The true set is already stated authoritatively at `SYSTEM_01:495-504` and in the middleware doc comment (`userPermission.ts:46-52`): the run-keyed figure-data reads, the authoring context, the ready-package list, the product email send, the copilot proxy and its Files route, plus the collab socket inline. (2) `SYSTEM_02_persistence.md:518-521` says "three more siblings are missing from the instance and project barrels (named in the Conventions section above)"; the Conventions text at `:472-479` names four, `dataset_iceh.ts`, `run_generation.ts` and `user_logs.ts` for `instance/mod.ts` and `visualization_folders.ts` for `project/mod.ts`, all four confirmed against the barrels. The same sentence's "`db/mod.ts` re-exports `postgres/mod.ts`, `utils.ts`, `instance/mod.ts` and `project/mod.ts`, each of which aggregates most of its siblings" reads `utils.ts` as a barrel; it is a leaf module. |
| Review 4, second pass, cosmetic (not in the Fix 4 list) | The reflows left three ragged short lines inside otherwise 80-column paragraphs: `SYSTEM_08_results_packages.md:430` ("`requireApprovedUser()` (D7). The project") and `:445` ("attached run,"), and `SYSTEM_09_viz_query_cache.md:631` ("renders the pinned form"). No claim is affected; fold them in with the two findings or leave them. |
| Step 4 fixed, second pass | Both findings of the second review re-checked in the code and fixed. (1) `SYSTEM_01_api_contract.md:638-640` no longer says every product and package read sits behind `requireApprovedUser()`: a route that needs the flag takes `requireApprovedUser()` (the set named at `:495-504`) or `requireProductAccess(level)`, whose policy checks it. The four `can_view_data` package-internals reads are confirmed at `server/routes/instance/run_generation.ts:177,197,210,222`. (2) `SYSTEM_02_persistence.md`: the Conventions bullet names `utils.ts` as a leaf and the other three as barrels (`server/db/mod.ts`), and the Open item says four siblings, not three. The three ragged lines in `SYSTEM_08_results_packages.md` and `SYSTEM_09_viz_query_cache.md` are reflowed. The em-dash in the diff is the quoted UI string `"{Area} — All areas"`, moved by the reflow and not added. Gates, reduced by Tim's ruling because the change is prose in four SYSTEM files and touches no manifest: `deno task typecheck` PASS (server, client, `lint:systems`), `deno task test` 165 passed 0 failed. The migration, consolidation, fresh-boot, query and protocol validators and `build:help-buttons` were not run, because none of them reads `SYSTEM_*.md`. |
