...
## Merge execution: main → results-runs (IN PROGRESS on `results-runs-merge`, started 2026-07-28)

**This section was authored on `main` (2026-07-26) as merge-execution
guidance and carried forward at the merge per its own instruction. Delete it
once the merge is done and the rig is green on the merged tree.** Measurements
below re-taken 2026-07-28 at merge start.
...
- **Divergence:** merge-base `d0ee2e3e` (2026-07-09). Branch = 30 commits /
  173 files; `main` = 335 commits / 790 files; **67 files overlap**.
- **Real conflict set** (re-measured 2026-07-28 by executing the throwaway
  ...
  **33 files — 25 content conflicts, 7 branch-deleted/
  main-modified, 1 main-deleted/branch-modified.** Hotspot:
  `server/routes/project/presentation_objects.ts`. Breakdown:
  7 docs, 13 `client/`, 1 `lib/` (`lib/types/modules.ts`), 12 `server/`.
  Drift vs the 2026-07-26 sizing: `SYSTEM_09`/`SYSTEM_11` now auto-merge
  (hand-verify them); newly conflicting: `t4_ui.ts`,
  `_main_database_types.ts`, `caches/visualizations.ts`,
  `load_module.ts`, `PresentationObjectMiniDisplay.tsx`.
...
conflict cost is already inside the measured 33-file set above. Snapshot
