...
## Status: build DONE on `results-runs` (exit gate 2026-07-14); main MERGED 2026-07-28 on `results-runs-merge` — **START AT "Merge execution" BELOW** (search "Remaining work, in order") for the current to-do list; this Status block is the build record
...
4. **Dual-write is the rollback path.** Wizard execution keeps ingesting into
   the project's legacy `ro_*` tables (today's COPY, unchanged) until the fleet
   is verified. Rollback = redeploy the previous image: the pg read path serves
   current data because the dual-write kept it current, and the parity rig keeps
   its pg baseline the same way. After fleet verification, the dual-write, pg
   read path, and legacy ingest are deleted (Phase 3 entry).
...
8. **Pre-deploy review work items — DONE 2026-07-14** (gates green: typecheck +
   ...
   - **Rig 16 (synthetic corpus, in-rig only, never stored)**: per metric,
     ...
     periodBounds), and a non-default replicant pane. Unbuildable fetch configs
     drop the variant (counted + logged). Composition is printed in TOTALS; this
     run: rollup=30 facility=17 replicant=12 + all 7 periodFilter kinds
     (custom=38, from_month=25, last_n_months=33, quarter×2=66, year×2=98).

Exit gate — PASSED 2026-07-14: `deno task typecheck` + the HARDENED rig PARITY
GREEN in `--run` mode (668 checks, 0 diffs/both_error/skips) → trial-instance
rollout per Deploy phasing (Ethiopia early).
...
## Merge execution: main → results-runs — CONFLICTS RESOLVED, semantic batch + rig outstanding

**State (2026-07-28):** the merge is committed as `c143e23e` on the throwaway
branch **`results-runs-merge`** (rerere recorded). `results-runs` is pristine
(fast-forward only after the rig is green); **`main` is not to be touched until
Tim says so.** All 33 conflicts are resolved; client `tsc` is clean; server
`deno check main.ts` has **exactly one** deliberate error (step 2 below). The
gates have NOT been run. The subsections below this handoff ("Verify
everything", "Merge pre-flight", "Rulings", "How to run it") are the original
guidance, kept as reference — their conflict- resolution steps are EXECUTED; the
parity-rig and inherited-defect subsections are still live. Delete this whole
merge section once the rig is green on the merged tree.

### Decisions taken at the merge (ratified by Tim 2026-07-28)

- **`PO_CACHE_VERSION` = "9", not the ruled "8"** — main had independently
  minted "8" (rollupDim) after the ruling; "bump past BOTH sides" held.
  **`po_detail` → `po_detail_v4`** as ruled (note both sides had minted a
  divergent "v3"). Both applied in `server/routes/caches/visualizations.ts`.
- **Batch period-filter route**: the virtual-default guard now runs BEFORE any
  live-room application, so room merges and the DB write share one id set — the
  "Inherited defect" below is fixed on this surface (its client half, menu
  gating, was not touched).
- **AI views**: `viewing_data`/`viewing_modules` deleted with their tabs;
  **`viewing_results_package`** added (`PROJECT_TAB_TO_VIEW` is
  `Record<TabOption, …>`, so the map forced a decision). Module AI tools survive
  — the branch had already re-pointed them to run reads.
- **Moved-home re-apply**: only ONE of main's edits to branch-deleted files had
  surviving markup (`_shared/module_parameter_inputs.tsx`, `text-neutral` →
  `text-base-content-muted`); the wizard rebuilt the rest fresh, nothing else to
  carry.
- **Resolution pattern everywhere else**: branch's run-based reads/virtual
  defaults/cache re-key + main's collab chokepoints
  (`applyPoToLiveRoom`/`closePoRoom`), n-values (`QueryContext.hasFacilityId`,
  `exceedsMaxReplicantOptions`), blank-sentinel, `trackStore` deep-tracking
  (composed with the branch's `runVersionKey` tracked read).
  `db/project/modules.ts` resolved to byte-identical with the branch (verified
  by diff).
- **Docs**: SYSTEM_01/03/06/15 reconciled; SYSTEM_08 rewritten to the runs model
  at section level (full rewrite stays Phase 4); SYSTEM_09 carries an updated
  drift banner (its cache-key tables are stale by design — S3's catalog is
  live); SYSTEM_11's glob delta survived the auto-merge.

### Remaining work, in order (the fresh agent's list)

1. **Manifest schema** (`lib/types/run_manifest.ts` +
   `server/runs/synthesize_run.ts`): (a) store the facilities input column types
   — `exportPgTableToParquet` already returns `{name, duckDbType}[]` for
   `facilities_hmis`/`facilities_hfa` and `buildRunPackageIntoTmp` currently
   discards it; (b) add the per-metric `datasetFamily` stamp (derive via lib
   `getDatasetFamily` from the stored module definition at build time — main
   added the field to `ResultsValue` for the HFA-only sample-size editor
   toggle). Make both REQUIRED and bump `RUN_MANIFEST_SCHEMA_VERSION` — runs
   exist only on dev, and re-backfill (step 6) regenerates them all.
2. **`buildQueryContextFromManifest`** (`server/run_query/run_read.ts` ~205) —
   the one remaining typecheck error: `hasFacilityId` from the RO's manifest
   stamp (`ro.hasFacilityId`, already present); `textColumns` = RO columns with
   `duckDbType === "VARCHAR"` plus, when `needsFacilityJoin`, the family's
   facilities columns from step 1a. This is what makes main's blank-fold SQL
   (`query_helpers.ts`, merged silently, +296 lines) behave identically on
   DuckDB.
3. **`datasetFamily` read path**: thread the stamp through
   `getMetricsWithStatusFromManifest` / the metric-info payload
   (`getResultsValueInfoFromRun`) so `ResultsValue.datasetFamily` is populated
   from the manifest exactly as `metric_enricher.ts` (the pg/rig baseline,
   auto-merged with the field) computes it live.
4. **Migration renumber** (Tim's ruling, unchanged): instance `056_add_runs.sql`
   → `065_…`, `057_run_generation.sql` → `066_…`; project
   `030_delete_default_visualization_rows.sql` → `038_…`. The runner keys by
   filename — reconcile dev by hand (insert new ids into `schema_migrations` or
   drop + re-apply). Verify the auto-merged `_main_database.sql` unions BOTH
   sides' tables (runs + run_generation_attempts AND import
   ledger/credentials/recurrence), then `validate_migrations` + a dev boot.
5. **Regenerate `deno.lock`** — don't trust its textual auto-merge.
6. **Re-backfill dev runs** (`backfill_runs.ts`, flags below) so all manifests
   carry the step-1 fields; clear dev IndexedDB when testing (dev has no deploy
   flush).
7. **Gates**: `deno task typecheck` (includes `lint:systems` — it will flag any
   glob left pointing at a merge-deleted file); verify-by-executing the silent
   auto-merges (`get_script_with_parameters_hfa` — generate a script;
   `t1_sse.tsx`).
8. **The extended parity rig — THE exit gate** (details in the parity-rig
   subsection below): extend the synthetic corpus with blank-value filters,
   multi-membership dimensions, and an HFA table config with `showNValues`
   exercising `__n_*`/`COUNT(DISTINCT …) FILTER` (HFA-only and new — the
   stored-PO corpus has none), then green fleet-dev:
   `deno run --allow-all --unstable-broadcast-channel --env-file -c
   deno.json validate_results_runs_parity.ts --run`
   (same flags for `backfill_runs.ts`; `./pg_run` starts dev Postgres with
   sandbox AND runs volumes; server/lib have no --watch — harness-execute
   directly).
9. **Only then**: delete this merge section,
   `git checkout results-runs &&
   git merge --ff-only results-runs-merge && git branch -d
   results-runs-merge`.
   Do NOT merge to or touch `main` — Tim decides the rollout (trial instance →
   rig there → fleet, Ethiopia early).

Context that carries: wb-fastr-modules HEAD `004fdc2` contains both workstreams;
its 4 unpushed commits ride the deploy (three-repo lockstep). The known rig
trap: a wizard run that selected a SUBSET of modules leaves that project
attached to a legitimately narrower package than the pg baseline — remedy is
re-backfilling that project, not a code fix.
...
- The "no conflict in the SQL builders" claim is now **FALSE** (n-values landed
  2026-07-26, `eac81c66`): all six `server_only_funcs_presentation_objects/`
  files conflict, and `query_helpers.ts` (+296 lines on `main`) still merges
  silently — see the parity-rig section.
...
### Merge pre-flight (re-measured 2026-07-28 at merge start; sizing merge RE-EXECUTED)

- **Divergence:** merge-base `d0ee2e3e` (2026-07-09). Branch = 30 commits / 173
  files; `main` = 335 commits / 790 files; **67 files overlap**.
- **Real conflict set** (re-measured 2026-07-28 by executing the throwaway
  `git merge --no-commit --no-ff main` on a scratch branch, then aborting): **33
  files — 25 content conflicts, 7 branch-deleted/ main-modified, 1
  main-deleted/branch-modified.** Hotspot:
  `server/routes/project/presentation_objects.ts`. Breakdown: 7 docs, 13
  `client/`, 1 `lib/` (`lib/types/modules.ts`), 12 `server/`. Drift vs the
  2026-07-26 sizing: `SYSTEM_09`/`SYSTEM_11` now auto-merge (hand-verify them);
  newly conflicting: `t4_ui.ts`, `_main_database_types.ts`,
  `caches/visualizations.ts`, `load_module.ts`,
...
- **Merge `main` into `results-runs`. Do not rebase.** A rebase replays 30
  commits across 272, so the same conflict surface is resolved up to 30 times; a
  merge is one pass and preserves the commit narrative this plan cites by hash.
  Enable `git rerere` either way.
- **Doc conflicts resolve toward `main`:** the branch still carries
  ...
  edit since merge-base is one line; carry it into `SYSTEM_08_module_system.md`.
  The five plan files auto-resolve to `main`'s deletions:
...
### Rulings (2026-07-26; cache-knob ruling superseded by the handoff above — "9"/v4 applied)

- **Renumber the branch migrations onto `main`'s tail** (Tim's ruling): instance
  `056_add_runs.sql` → `065_add_runs.sql`, `057_run_generation.sql` →
  `066_run_generation.sql`; project
  `030_delete_default_visualization_rows
  .sql` → `038_…` (same policy —
  `main`'s project tail is 037). The runner keys by filename, so renaming makes
  dev re-run them under the new ids — accepted; reconcile dev by hand (insert
  the new ids into `schema_migrations`, or drop the objects and let them
  re-apply). Verify the auto-merged `_main_database.sql` actually unions both
  sides' tables (runs + run_generation_attempts AND import
  ledger/credentials/recurrence), then run `validate_migrations` and a dev boot.
- **Bump both cache knobs past BOTH sides**: `PO_CACHE_VERSION` is "7" on `main`
  and "6" on the branch (independent bumps for different payload changes) →
  merged tree takes **"8"**; `po_detail_v2` (`main`) vs `v3` (branch) →
  **`v4`**. Either side's number alone serves mixed-shape payloads. Clear dev
  IndexedDB when testing (dev has no deploy flush).
- **Regenerate `deno.lock`** after the merge; don't trust its textual
  auto-merge.
- **Moved-file re-apply class** — the easiest thing to drop silently: 6 of the 7
  branch-deleted client files are files the branch MOVED (windowing/ params
  editors extracted for wizard reuse, e.g. `settings_generic.tsx`'s param grid →
  `_shared/module_parameter_inputs.tsx`). `main`'s 1–16-line edits to the old
  paths must be re-applied to the new homes by hand — git resolves the delete
  and drops them. `project_data.tsx` is a true delete: its ~460 changed lines on
  `main` are the staleness/attach surface the wizard replaces; nothing there
  needs a wizard home.
- **wb-fastr-modules is ready**: HEAD `004fdc2` contains both workstreams (the
  branch's pinned-asset `6ba142e` is an ancestor; `showNValues` +
  drop-facility_name landed on top), 4 unpushed commits ride the deploy. The
  github-definition schema conflict (1 hunk in
  `lib/types/_module_definition_github.ts`) resolves as a **union**
  (`RepoAssetPin` + the new optional fields). The branch touches zero panther
  files — `main`'s panther syncs merge clean.

### How to run it — in-place in the primary checkout, NOT a worktree (steps 1–5 EXECUTED 2026-07-28; steps 6–9 = the handoff list above)
...
**Measured conflict surface** (31 files): 7 docs, 12 `client/`, 1 `lib/`, 11
`server/`; root/config auto-merges.

1. **Land the tree first.** Commit outstanding work on `main` — commit, do not
   stash: this is a multi-hour branch-switch and a forgotten stash is a footgun.
2. **Turn on conflict memory:** `git config rerere.enabled true`. Resolutions
   are then replayed if a step is redone.
3. **Sizing — RE-EXECUTED 2026-07-28**; results are the "Real conflict set"
...
5. **Resolve in this order** — cheapest and least risky first, so the remaining
   ...
   1. **Deletions** — only `DOC_MODULE_EXECUTION.md` needs a decision (one-line
      branch edit → carry into SYSTEM_08); the five plan files auto-resolve to
      `main`'s deletions.
   2. **Docs** (7 `.md`) — take `main`'s structure, re-apply the branch's runs
      ...
      carry this merge-guidance section forward, and fix its stale claims in the
      same commit.
   3. **`lib/`** (1) — `lib/types/modules.ts`; the definition-schema union is in
      the rulings above.
   4. **`server/`** (11) — hotspot `routes/project/presentation_objects.ts` (15
      hunks); the six `server_only_funcs_presentation_objects/` files get real
      review — both sides rewrote them (branch: engine seam; `main`: blank
      filters, multi-membership, n-values).
   5. **`client/`** (12) — the 7 delete/modify files resolve to "deleted", THEN
      re-apply `main`'s small edits to the moved copies (rulings above).
6. **Post-resolution semantic work** (not optional — the merge is not done
   ...
   renumber 065/066 + project 038; `PO_CACHE_VERSION` → "8", `po_detail` → `v4`;
   regenerate `deno.lock`; `_main_database.sql` union check.
7. **Gates**: `deno task typecheck` (includes `lint:systems`, which will flag
   ...
   `validate_migrations`; a dev boot (migrations apply cleanly); re-backfill dev
   runs (`backfill_runs.ts`) so manifests carry the new fields; verify the
   auto-merged `t1_sse.tsx` and `get_script_with_parameters_hfa.ts` by executing
   (generate a script), not by reading.
8. **Run the parity rig, extended** — see the next section. This is the gate.
9. **Only then** fast-forward:
   `git checkout results-runs && git merge --ff-only
...
`main` rewrote the exact SQL surface this plan re-executes on DuckDB (+360/−90
across `server_only_funcs_presentation_objects/`):
...
- `eac81c66` — table sample sizes: **`COUNT(DISTINCT …) FILTER (WHERE …)` is now
  IN the generated SQL** (`query_helpers.ts:500`, emitting `__n_*`). §2.4's
  dialect inventory recorded FILTER as absent; that inventory is stale. DuckDB
  supports the syntax (and the `::int` cast sidesteps BigInt), but nothing has
  verified it through the adapter.
...
flows straight into DuckDB execution. **Blank-value filtering is NULL semantics
(`''` vs NULL), precisely where DuckDB/Parquet and Postgres diverge** (§2.4
already flags `nullstr`, binary-vs-collated ORDER BY, and integer division).

So run `validate_results_runs_parity.ts` against the merged tree and treat THAT
as the gate — after **extending the corpus** with: blank-value filters,
multi-membership dimensions, and an HFA table config with `showNValues`
exercising `__n_*`/FILTER (n-values is HFA-only and new, so the stored-PO corpus
won't contain one). The dual-write keeps the Postgres baseline live for exactly
this purpose.
...
conflict cost is already inside the measured 33-file set above. Snapshot naming
and the geojson snapshot did NOT land (verified: `t1_store.ts` untouched since
merge-base). **Nothing further lands on `main` before the merge.**

### Known incoming rework — NOW DUE (n-values landed as `eac81c66`; = handoff steps 1–3 above)

Part of the merge itself, beyond conflict resolution: **`datasetFamily` →
manifest stamps.** `main` added the field to
[metric_enricher.ts](server/db/project/metric_enricher.ts) for an editor
affordance; the branch demotes that probe-based enricher to the parity rig's pg
baseline and serves reads from manifest lookups. Move the field into the
...
affordance silently breaks on the run read path. Then re-backfill dev runs so
existing manifests carry it. (Accepted and recorded in SYSTEM_09 Open items.)
The FILTER/`__n_*` rig case is in the parity-rig section above.

### Inherited defect — batch period-filter vs default visualizations

Carried over from the 2026-07-27 regression sweep, deferred here on the grounds
that this plan reworks the surface (2026-07-27 ruling).

`batchUpdatePresentationObjectsPeriodFilter`
([presentation_objects.ts](server/db/project/presentation_objects.ts)) returns
`{success:false, err:"You cannot update a default visualization"}` if **any** id
in the batch is a default — all-or-nothing, where pre-1b4ec928 the batch applied
to all. The client offers "Edit common properties…" on every multi-selection
(the card menu gates on `isMultiSelect` only, unlike "Move to folder", which
gates on `isDefault`), including the `_defaults` group and the mixed selections
the `module`/`metric` grouping modes produce.

Worse, the route
([routes/project/presentation_objects.ts](server/routes/project/presentation_objects.ts))
applies the change into live collab rooms for **all** ids _before_ calling the
batch, so a mixed batch partially applies — room-backed POs updated and
checkpointed — and then reports an error saying nothing happened. Defaults never
hold a room themselves (`collabEnabled` excludes them), so the partial write
always lands on the non-default members.

Whatever replaces this path must apply live-room updates and the DB write under
the SAME id set, decided AFTER the default guard. Verified 2026-07-27. **Server
half FIXED at the merge (2026-07-28)**: the route now runs the virtual-default
guard before any room application, so rooms and the batch write share one id
set. The client half — "Edit common properties…" still offered on every
multi-selection including defaults (card menu gates on `isMultiSelect` only) —
remains as-is; the server now refuses cleanly with no partial apply.

### Merge ≠ rollout

Resolving conflicts and getting the rig green is roughly **1.5–2 focused days**
(revised up from "a focused day": the six SQL-surface conflicts, the
`datasetFamily` manifest work, and the rig extension are real work on top of the
~50 mechanical hunks) and is agent-assistable. The rollout that follows (trial
prod instance → backfill → rig there → fleet, Ethiopia early as the
...
3. **Raw CSV retention** — **RESOLVED 2026-07-12 (Tim)**: keep raw CSVs in runs
   ...
   anyway.
4. **Scheduled auto-runs** (import → generate → auto-repoint): Phase 3 scope or
   later? Auto-repoint in particular changes what "immutable attachment" means
   for a project.
