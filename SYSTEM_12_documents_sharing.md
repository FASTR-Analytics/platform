...
`folder_tree.ts` derivations and their harness, the tree list view, the two
...
two logo editors are this system's under
...
opens on the SSE echo, on the page's own `awaitingProductId` wait for a
product it just created, or through `pendingEditorOpen({ kind: "product" })`,
the one opener the tours, the copilot and the `?product=<id>` deep link share
...
line, the same two text boxes the AI writes. A headingless column grows a ghost
"Heading…" (`attachColumnHeadingGhost`), like a missing kicker: while its
region is active in the live editor, on hover on the pages. Typing into it
and pressing Enter (or leaving it) inserts a `### ` line under the `:::col`
fence; a heading inside a block emptied of its words (with or without its
hidden marker) loses its line on blur, so the ghost returns. STABLE TEXT METRICS
...
**Conditional formatting never re-tints** (reverted 2026-09-25 at a
colleague's request: the completeness table must show the standard green,
yellow and red under every theme). A thresholds or scale format renders with
its own colours in a report exactly as on a dashboard; the palette reaches
`getLegendFromConfig` only so the special charts' legend swatches match what
their builders draw. A figure's explicit per-series colours are never replaced either. The
