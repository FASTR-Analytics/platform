// =============================================================================
// Figure-config CRDT bridge (Yjs): a figure node inside a slide or report
// =============================================================================
//
// A figure's editable config is `PresentationObjectConfig = { d, s, t }`. This
// module maps it onto a Y.Map so two people editing the SAME config in the
// figure editor merge field-by-field (different fields never clobber), with
// same-field last-writer-wins and character-level merge on the three free-text
// fields. It is a bridge over a Y.MAP (not a whole Y.Doc): the map it binds is
// the figure node's "figConfig" nested map inside the slide or report doc.
//
 ...
 *  (the figure editor's full-store push). */
