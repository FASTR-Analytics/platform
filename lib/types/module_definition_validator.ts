...
const translatableString = z.object({
  en: z.string(),
...
const moduleParameterInput = z.discriminatedUnion("inputType", [
  ...
  z.object({ inputType: z.literal("boolean"), defaultValue: z.enum(["TRUE", "FALSE"]) }),
  z.object({
...
const postAggregationExpression = z.object({
  ingredientValues: z.array(z.object({
    prop: z.string(),
    func: z.enum(["SUM", "AVG"]),
  })),
  expression: z.string(),
...
const periodFilter = z.object({
  filterType: z.enum([
    "last_n_months", "from_month", "last_calendar_year", "last_calendar_quarter",
  ]).optional(),
...
const configS = z.object({
  ...
  colorScale: z.enum(["pastel-discrete", "alt-discrete", "red-green", "blue-green", "single-grey", "custom"]),
  decimalPlaces: z.union([z.literal(0), z.literal(1), z.literal(2), z.literal(3)]),
  ...
  customSeriesStyles: z.array(z.object({
    color: z.string(),
    strokeWidth: z.number(),
    lineStyle: z.enum(["solid", "dashed"]),
  })),
  nColsInCellDisplay: z.union([z.literal("auto"), z.number()]),
  seriesColorFuncPropToUse: z.enum(["series", "cell", "col", "row"]).optional(),
...
const vizPreset = z.object({
  ...
  defaultPeriodFilterForDefaultVisualizations: z.object({
    nMonths: z.number(),
  }).optional(),
  config: z.object({
    d: configD,
...
export const ModuleDefinitionJSONSchema = z.object({
  label: translatableString,
