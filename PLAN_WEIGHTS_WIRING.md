...
3. **Zero/blank/NULL weights (DECIDED 2026-06-11)** — stored weights are strictly positive (importer + DB CHECK, migration 051). A **blank** weight cell means the facility is not in that round's sample: the importer skips the row (reported in the result) and stores nothing — absence of a weight row is the representation, and a not-sampled facility has no data rows for that round anyway. A literal **0** is rejected as a data error (design weights are 1/π ≥ 1; a 0 would silently exclude the facility). **NULL** is never stored — under the script's `coalesce(weight, 1)` fallback a NULL on a data row would be silently *included* at weight 1, the opposite of what a NULL would be trying to say. Ruled out: there is no surveyed-but-excluded-from-sample case in these surveys, so no exclusion marker exists.
