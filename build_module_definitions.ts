...
import type { ResultsValueForVisualization } from "./module_definitions.ts";
...
export function getResultsValueForVisualizationFromMetricId(metricId: string): ResultsValueForVisualization {
  const data = getMetricStaticData(metricId);
  return {
    formatAs: data.formatAs,
    valueProps: data.valueProps,
    valueLabelReplacements: data.valueLabelReplacements,
  };
}
`;
}
