...
| 4 | Finding: `products/list_view.tsx:44-48` colours the two sortable headers by state, `text-base-content` when sorted and `text-base-content-muted hover:text-base-content` when not, while the DisplayTable header (`panther/_303_components/tables/display_table/table.tsx:283`) keeps every header `text-base-content` and marks the sorted column with a glyph (`SortIcon`, `table.tsx:418-432`: `HeaderGlyph` at `opacity-40` when inactive). The build-log claim that the sorted column is marked by colour against muted holds only against the other sortable header: Type, Package and Scope inherit the same base colour as the sorted header, so the one header that differs is the unsorted sortable one, not the sorted one. Ruling 8 says the listing's column header matches the DisplayTable header. Resolution: every header in the base colour (drop the ternary, keep `ui-focusable cursor-pointer text-left` and the hover), and mark the sorted header as DisplayTable does, with an `Icon` glyph after the label, muted (`opacity-40`) on the unsorted sortable header. `HeaderGlyph` is not exported from `display_table/mod.ts`, so it is rebuilt from `Icon` here, which the file's own comment allows; `SortMode` has no direction, so one arrow glyph for the active header suffices. |
| 4 | Step 4 reviewed: 1 finding. Surface held: `24f19d42` touches 10 files (9 under `client/src/components`, `client/src/routes/access_tokens.tsx`), each a `Table` with `TableColumn` renders; `db4ff665` touches `products/list_view.tsx` and this plan (2 in `git show --stat`; the built row's "1 file" counts the code file). Ruling 7 verified in the kit: `data_grid.tsx:133` `ui-text-small`, `table_from_csv.tsx:47,105,111` `ui-text-small`, `presence_grid.tsx:32` `text-xs`; the nine app usages pass no class and the three components take no children, so the `ui-text-caption` at `explore/data_table/grid.tsx:41` and the `text-sm` at `hmis/population/grid.tsx:268` are siblings, not cells. Ruling 8, display tables: every `render:` in the 23 files that use `Table` is size-free on its main value; every remaining `text-sm` or `text-xs` in those files is page text outside a table. The stacked lines at `hmis/indicators/manager.tsx:517` and `dhis2_tab_future.tsx:145,185` are `ui-text-caption` (the latter with `text-danger` beside it, a utility over the component class as `step_4.tsx:152` does). Judged not a finding: the HMIS status cell (`manager.tsx:545-556`) can stack a `text-warning` population line under a `text-danger font-700` problem line at body size when a judgement is neither computable nor unresolvable (`manager.tsx:325-350`), but each line is the cell's main value when alone, so neither is a secondary line. Results list: `results_packages.tsx:246-247` is a bold body label over a caption line, and it has no column header to match. Products list: the cells at `list_view.tsx:137-148,188-201` carry no size, folder counts and dates are `text-base-content-muted`, the folder name is `font-700`; the header at `:218` is `font-700 text-xs uppercase tracking-wider`, the DisplayTable header less `text-base-content`, which it inherits. Floor: typecheck exit 0, tests exit 0 (428 passed, 2 ignored), protocols exit 0 (16 baselined). |
| 5 | Ruling 11: every `text-[9px]`, `text-[10px]` and `text-[11px]` became `text-xs` (12 sites in 10 files, including the class string built in `live_preview_extension.tsx`). The diff-segment flag's inline 10.5px style became `text-xs`. |
| 5 | Ruling 1 in the editors: the collab markdown editor reads `var(--text-sm)` (was 14px) and its hover flag `var(--text-xs)` (was 11px); the report body editor reads `var(--text-base)` (was 15px), which also matches the 16px the report page surface renders at. |
| 5 | Exempt from the lint, by file, as document and canvas rendering: `products/report/live_preview_extension.tsx` (the report page surface is pinned to its PDF's typography at 16px) and `products/report/fastr_theme_mock.tsx` (the theme miniature, drawn to a tile). The `0.7em` page-break marker in `paged_edit_surface.ts` is an `em` size, which §2 allows, and the lint does not flag `em`. |
| 5 | The lint (`lint_text_sizes.ts`, task `lint:text-sizes`) is chained into `deno task typecheck` after `lint:structure`. Docs that describe the typecheck chain were updated in this step (`CLAUDE.md`, `README.md`, `PROTOCOL_APP_PLANS.md`, `PROTOCOL_APP_DEVELOPMENT.md`, `SYSTEMS.md`, `SYSTEM_15_admin_ops.md`); they are outside the step's Surface as written, but the protocol says docs move with the code. |
| 5 | Step 5 built. Commits: "Every UI text size is a rem token" (2b48ef4c, 13 files), "Lint every UI text size against the token scale" (8 files). Floor after each: typecheck exit 0 (now including the lint), tests exit 0, protocols exit 0. |
| 4 | Fix: every Products list header is in the base colour; the sortable headers carry a glyph after the label as DisplayTable's SortIcon does, `arrowDown` on the sorted column and `arrowsUpDown` at `opacity-40` on the other. `SortMode` has no direction, so one arrow suffices. |
| 4 | Fix, second pass on Tim's direction: the two sortable Products headers are DisplayTable-style header buttons (`ui-hoverable-base-100 ui-focusable -ml-1.5 inline-flex items-center gap-1 rounded px-1.5 py-1 uppercase`, hover token for the base-100 surface they sit on) with the glyph inside the button; the active glyph is `arrowUp` for name (ascending) and `arrowDown` for recent (newest first). Non-sortable headers get the same `-ml-1.5 px-1.5 py-1` span so labels align. Per-column sort with direction and column filters like DisplayTable's are follow-up work after this plan, since they change the sort model and need a panther export. |
| 4 | Outside the plan, on Tim's direction during the fix, two commits sit in this step's range: "Products sort carries a direction; a second header click reverses it" (6a3f95cf: `lib/types/sort.ts` gains `ListSort`, the products sort state is `productsSort`, the header row gets `pt-1`) and "Remove the product type filter from the Products page" (its Select, state, tree filter, folder-count filter, tour step and docs). Neither is step 4 work; both are recorded here so the review can tell them from the fix. |
| 4 | Step 4 fixed. 1 file. Floor: typecheck exit 0, tests exit 0, protocols exit 0. |
| 4 | Finding, log only, outside the plan: `cc095beb` removed the product type filter but left two docs describing it. `PROTOCOL_APP_UI_CONVENTIONS.md:134` still names "the type-filter `Select` in `centerChildren`" and `:136` "open folders / sort / type-filter state"; `SYSTEM_14_client_shell.md:251` still says "the product explorer's three" while listing the two that remain. The commit is Tim's own direction, so the plan does not carry the doc edit; no code changes. |
| 4 | Step 4 reviewed: 1 finding. Range `13f36b6c..HEAD`: `4d0f3fa3` and `9e3c8072` (the fix, both `products/list_view.tsx` and this plan), `6a3f95cf` and `cc095beb` (outside the plan, as the §8 row says), `614a29a5` (plan only). The review's finding is resolved: the header row (`list_view.tsx:227`) carries no colour class, so every header inherits the base colour; the two sortable headers are buttons (`:49-62`) with `ui-hoverable-base-100 ui-focusable -ml-1.5 inline-flex cursor-pointer items-center gap-1 rounded px-1.5 py-1 uppercase`, DisplayTable's `HEADER_BUTTON` (`table.tsx:36-37`) with the hover token for the base-100 surface, and the glyph inside the button: `arrowUp` or `arrowDown` by direction when active, `arrowsUpDown` at `opacity-40` otherwise (`:40-45,55-60`), as `SortIcon` and `HeaderGlyph` do (`table.tsx:418-432`, `header_glyph.tsx:14-19`); Type, Package and Scope carry the `-ml-1.5 px-1.5 py-1` span (`:233,236,239`). The two outside commits are what the row says: `6a3f95cf` adds `SortDirection` and `ListSort` to `lib/types/sort.ts`, replaces `productsSortMode` with `productsSort` (`t4_ui.ts:117-135`, parsed defensively, default recent/desc), adds `defaultSort` and `nextSort` and a direction sign to `sort_by_sort_mode.ts`, rewires `products.tsx` and `product_menu.ts`, updates `SYSTEM_14` and adds `pt-1` to the header row; `cc095beb` removes the Select, `_ALL_TYPES`, `productsTypeFilter`, the `typeFilter` arm of `buildProductTree`, the folder-count filter, the tour step and its `productsTypeFilter` dependency, `pluralLabel` (the Select's only consumer) and the type-filter tree test, and edits `SYSTEM_12` and `SYSTEM_14`. No `productsSortMode`, `productsTypeFilter`, `typeFilter`, `_ALL_TYPES`, `pluralLabel` or `products-type-filter` remains under `client/src`, `lib` or `server`. `sortBySortMode` (`sort_by_sort_mode.ts:15-37`) multiplies the name and date comparisons by the direction sign and returns an undated item last before the sign applies, so `desc` reverses and undated items stay last either way. Floor: typecheck exit 0, tests exit 0 (427 passed, 2 ignored; one fewer than before, the removed type-filter test), protocols exit 0 (16 baselined). |
