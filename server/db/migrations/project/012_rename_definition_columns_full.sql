...
DO $$
BEGIN
  -- Rename script_updated_at -> compute_def_updated_at (if exists from older schema)
  IF EXISTS (
  ...
  -- Rename compute_updated_at -> compute_def_updated_at (if exists from intermediate schema)
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
  ...
  -- Rename definition_updated_at -> presentation_def_updated_at
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
  ...
  -- Rename installed_git_ref -> presentation_def_git_ref
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
  ...
  -- Rename compute_git_ref -> compute_def_git_ref (if exists from intermediate schema)
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
