...
type LedgerPair = { indicatorId: string; periodId: number };
...
function dedupePairs<T extends LedgerPair>(pairs: T[]): T[] {
  ...
  for (const p of pairs) {
    map.set(`${p.indicatorId}|${p.periodId}`, p);
  }
...
export async function upsertHmisLedgerPairsFromData(
  ...
  const indicatorIds = deduped.map((p) => p.indicatorId);
  ...
  // The indicators JOIN skips pairs whose indicator was deleted between
  // staging and integration (possible for pairs with no dataset_hmis rows:
  // deleting an indicator only refuses when it has data). Without it the FK
  ...
  await sql`
    INSERT INTO dataset_hmis_import_ledger
      (indicator_id, period_id, n_records, sum_count, skipped_values, skipped_values_sample,
       source, status, error, imported_at, version_id)
    SELECT s.indicator_id, s.period_id, agg.n, agg.sum, s.skipped_values, s.skipped_values_sample,
      ${source}, 'ready', NULL, now(), ${versionId}
    FROM UNNEST(
      ${indicatorIds}::text[], ${periodIds}::int[], ${skippedValues}::int[], ${skippedSamples}::text[]
    ) AS s(indicator_id, period_id, skipped_values, skipped_values_sample)
    JOIN indicators i ON i.indicator_common_id = s.indicator_id
    CROSS JOIN LATERAL (
      SELECT COUNT(*)::integer AS n, COALESCE(SUM(dt.count), 0)::bigint AS sum
      FROM dataset_hmis dt
      WHERE dt.indicator_id = s.indicator_id AND dt.period_id = s.period_id
    ) agg
...
export async function upsertHmisLedgerErrorPairs(
  ...
  const indicatorIds = deduped.map((f) => f.indicatorId);
  ...
  await sql`
    INSERT INTO dataset_hmis_import_ledger
      (indicator_id, period_id, n_records, sum_count, skipped_values, skipped_values_sample,
       source, status, error, imported_at, version_id)
    SELECT s.indicator_id, s.period_id, 0, 0, 0, '[]', 'dhis2', 'error', s.error, NULL, NULL
    FROM UNNEST(${indicatorIds}::text[], ${periodIds}::int[], ${errors}::text[])
      AS s(indicator_id, period_id, error)
    JOIN indicators i ON i.indicator_common_id = s.indicator_id
    ON CONFLICT (indicator_id, period_id) DO UPDATE SET
      status = 'error',
...
export async function reconcileHmisLedgerPairsAfterDelete(
  ...
  await sql`
    ...
    FROM UNNEST(${indicatorIds}::text[], ${periodIds}::int[]) AS s(indicator_id, period_id)
    CROSS JOIN LATERAL (
      SELECT COUNT(*)::integer AS n, COALESCE(SUM(dt.count), 0)::bigint AS sum
      FROM dataset_hmis dt
      WHERE dt.indicator_id = s.indicator_id AND dt.period_id = s.period_id
    ) agg
    WHERE l.indicator_id = s.indicator_id
      AND l.period_id = s.period_id
      AND agg.n > 0
  `;
  await sql`
    DELETE FROM dataset_hmis_import_ledger l
    USING UNNEST(${indicatorIds}::text[], ${periodIds}::int[]) AS s(indicator_id, period_id)
    WHERE l.indicator_id = s.indicator_id
      AND l.period_id = s.period_id
      AND NOT EXISTS (
        SELECT 1 FROM dataset_hmis dt
        WHERE dt.indicator_id = l.indicator_id AND dt.period_id = l.period_id
      )
  `;
}
...
export async function getDatasetHmisImportLedgerItems(
  ...
  return await tryCatchDatabaseAsync(async () => {
    ...
    >`
      SELECT indicator_id, period_id, n_records, sum_count, skipped_values, skipped_values_sample,
        source, status, error, imported_at, version_id
