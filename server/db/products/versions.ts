...
// mirror content already validated when it was written to the live tables,
// and a later schema change must not be able to fail the version write.
// Validation happens on the way OUT: restore and copy parse with the current
// schemas. The parse alone cannot cover a renamed key (Zod strip mode drops
// unknown keys), so every path that reads a snapshot out runs the shared
// figure-block transforms first.
...
export async function insertReportVersion(
  ...
  return await tryCatchDatabaseAsync(async () => {
    ...
    await mainDb`
      ...
      WHERE report_id = ${args.productId} AND id NOT IN (
        SELECT id FROM report_versions
        WHERE report_id = ${args.productId}
  ...
  return await tryCatchDatabaseAsync(async () => {
    const row = (
      await mainDb<DBReportVersion[]>`
        ...
        WHERE id = ${versionId} AND report_id = ${productId}
      `
    ).at(0);
    if (!row) {
...
export async function getReportVersionLineage(
  ...
  return await tryCatchDatabaseAsync(async () => {
    ...
    // version that the list shows as OLDER, reversing a diff step.
 ...
 *  path). Bumping the product's last_updated alone invalidates any stored
...
export async function copyReportFromVersion(
  ...
  return await tryCatchDatabaseAsync(async () => {
    const version = (
      await mainDb<DBReportVersion[]>`
        SELECT * FROM report_versions
        WHERE id = ${args.versionId} AND report_id = ${args.productId}
      `
    ).at(0);
    if (!version) {
      throw new Error("Version not found");
    ...
    const source = (
      await mainDb<{ config: string | null }[]>`
        SELECT config FROM reports WHERE id = ${args.productId}
      `
    ).at(0);
...
export async function insertSlideDeckVersion(
  ...
  return await tryCatchDatabaseAsync(async () => {
    ...
    `;
    await mainDb`
      DELETE FROM slide_deck_versions
      WHERE slide_deck_id = ${args.productId} AND id NOT IN (
        SELECT id FROM slide_deck_versions
        WHERE slide_deck_id = ${args.productId}
        ORDER BY created_at DESC, id DESC
        LIMIT ${VERSIONS_KEEP}
      )
    `;
...
export async function latestSlideDeckVersionHash(
  mainDb: Sql,
  productId: string,
): Promise<APIResponseWithData<{ hash: string | null }>> {
  return await tryCatchDatabaseAsync(async () => {
    const row = (
      await mainDb<{ content_hash: string }[]>`
        SELECT content_hash FROM slide_deck_versions
...
export async function getSlideDeckVersion(
  mainDb: Sql,
  productId: string,
  versionId: string,
  ...
  return await tryCatchDatabaseAsync(async () => {
    const row = (
      await mainDb<DBSlideDeckVersion[]>`
        SELECT * FROM slide_deck_versions
        WHERE id = ${versionId} AND slide_deck_id = ${productId}
      `
    ).at(0);
    if (!row) {
      throw new Error("Version not found");
 ...
 *  Pure so it is harness-testable; the route derives room handling from it
...
/** Slide ids are short nanoids whose uniqueness is only checked against LIVE
 *  rows: a snapshot slide that was deleted may have had its id reused by a
 *  slide in another deck, so re-inserting it verbatim would violate the PK
 *  and abort the restore forever. Replace any colliding toInsert id with a
 *  fresh one. Call BEFORE closing rooms: the colliding id's live room
 *  belongs to another deck and must not be touched. */
export async function remapCollidingSlideIds(
  mainDb: Sql,
  plan: SlideDeckRestorePlan,
): Promise<
  APIResponseWithData<{ plan: SlideDeckRestorePlan; remapped: number }>
> {
  return await tryCatchDatabaseAsync(async () => {
...
/** Deck restore, structural half: one transaction that deletes and
 *  re-inserts slide rows (ids taken verbatim from the plan, after
 *  remapCollidingSlideIds), restores every slide's snapshot sort_order, and
 *  restores the deck config and the product label. Configs of surviving
 *  (toUpdate) slides are NOT written here: the route applies them afterwards
 *  through the live-room chokepoint so co-editors follow the restore live.
 *  Checkpoints never write sort_order, so a straggler checkpoint after this
 *  transaction can only touch config. */
export async function restoreSlideDeckStructure(
  mainDb: Sql,
  productId: string,
  label: string,
  deckConfig: SlideDeckConfig,
...
export async function copySlideDeckFromVersion(
  ...
  return await tryCatchDatabaseAsync(async () => {
    const version = (
      await mainDb<DBSlideDeckVersion[]>`
        SELECT * FROM slide_deck_versions
        WHERE id = ${args.versionId} AND slide_deck_id = ${args.productId}
      `
    ).at(0);
    if (!version) {
      throw new Error("Version not found");
    ...
    await mainDb.begin((sql) => [
      ...
      sql`
        ...
        VALUES (${newProductId}, '', ${parsedDeckConfig})
      `,
      ...slides.map((slide, i) =>
        sql`
          INSERT INTO slides (id, slide_deck_id, sort_order, config, last_updated)
