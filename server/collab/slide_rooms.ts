...
// (Slide <-> Y.Doc bridge + slide_* wire messages). Every entry point takes the
// owning product (deck) first: it is the room key's leading segment.
...
export function applySlideUpdate(
  ...
  applyDocUpdate(productId, slideId, conn, updateB64, slideAdapter);
}
...
export function relayAwareness(
  ...
  relayDocAwareness(productId, slideId, sender, updateB64, slideAdapter);
}
...
export function unsubscribeSlide(
  ...
  unsubscribeDoc(productId, DOC_TYPE, slideId, conn);
}
...
export function flushSlideRoom(
  ...
  return flushRoomForDoc(productId, DOC_TYPE, slideId);
}
...
export function closeSlideRoom(
  ...
  closeRoomsForDoc(productId, DOC_TYPE, slideId, message);
