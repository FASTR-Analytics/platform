...
// The instance country is threaded into the figure-block sweeps so backfilled
// bundles carry the real countryIso3 (drives Nigeria admin-area relabelling and
// admin replicant labels). New captures read it from the live instance store;
// a boot sweep cannot, so it gets it here.
type InstanceMigrationFn = (
  tx: Sql,
  countryIso3: string,
) => Promise<MigrationStats>;

// Each runs in its own transaction, in order.
...
async function runInstanceDataTransforms(mainDb: Sql): Promise<void> {
  ...
  for (const { name, fn } of INSTANCE_DATA_TRANSFORMS) {
    try {
      ...
      await mainDb.begin(async (tx) => {
        stats = await fn(tx, _INSTANCE_COUNTRY_ISO3);
      });
