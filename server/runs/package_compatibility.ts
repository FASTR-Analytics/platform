...
import { figurePackageIssueForDimensions } from "lib";
...
// The summary's projection of the same set lib/figure_package_issue.ts
// derives from a config (the summary row carries no config).
...
function issueFor(
  ...
  const issue = figurePackageIssueForDimensions(
    po.metricId,
    requestedDisaggregationOptions(po),
    manifest,
  );
  return issue === null
    ? null
    : { presentationObjectId: po.id, label: po.label, ...issue };
}
