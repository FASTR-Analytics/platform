...
// TODO: add preset recipient emails here
const _HELP_EMAIL_RECIPIENTS = [
  "support@usefuldata.com.au",
  "nicholaswillmottse@gmail.com",
];
...
defineRoute(
  ...
  async (c, { body }) => {
    ...
    const userHtmlBody = feedbackType === "bug"
...
<div style="font-family: sans-serif; color: #333;">
  ...
  <p>${description.replace(/\n/g, "<br>")}</p>
    ...
    async function sendEmail(to: string, subject: string, plainText: string, html: string) {
      const res = await fetch("https://api.sendgrid.com/v3/mail/send", {
        method: "POST",
        headers: {
          Authorization: `Bearer ${_SEND_GRID_API}`,
    ...
    await sendEmail(userEmail, `We received your ${typeLabel.toLowerCase()}`, userPlainText, userHtml);

    // Send notification to preset recipients
    for (const recipient of _HELP_EMAIL_RECIPIENTS) {
      await sendEmail(recipient, `[FASTR] New ${typeLabel} from ${userEmail}`, internalPlainText, internalHtml);
    }
