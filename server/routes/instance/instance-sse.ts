  ...
  async (c) => {
    ...
    return streamSSE(c, async (stream) => {
      ...
      try {
        ...
        // Three per-message rules, all LIVE off the same re-derivation
        // (every `users_updated` passing through the forward loop carries
        // the full roster with permission rows, and the connection's own
        // email never changes, so re-finding it in each roster is
        // sufficient):
        ...
        //     applies to the starting payload. A revoked user stops receiving
        //     products on the next `users_updated` without a reconnect; the
