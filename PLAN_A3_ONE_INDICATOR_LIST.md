...
| 2026-09-11 | 4 | Fleet dumps: `ssh wb-server` was refused by this session's tool permissions, so gate 6 ran over the dev `main` database only (a dump before and after the hand resolution). §7's run over every instance's dump stays the rollout precondition. |
| 2026-09-11 | 4 | Dev database resolved by hand before 086, as ruling 12 prescribes for the fleet: `anc1` was a derived row under a special id (expression `measles1`), retyped to an empty base; `Bl9cq5ZpeU5` was mapped to both `anc4` and `measles1`, kept on `anc4` and removed from `measles1`. The pre-resolution dump is at `~/wb-fastr-dev-main-before-086.sql.gz`. |
| 2026-09-11 | 4 | Gate 6 by instance. dev (before resolution): FAIL, 086 fail-stopped on the derived special `anc1` and nothing else changed (old tables, columns, `is_default` and the migration record intact). dev (after resolution): PASS, 8 sources from mappings, 7 new bases from unmapped raws (`postnatal_clinic_visits`, `postnatal_clinic_visits_newborns_2_3d`, `postnatal_clinic_visits_newborns_4_7d`, `postnatal_clinic_visits_mothers_1d`, `postnatal_clinic_visits_newborns_7d`, `l4esisvd1hu`, `art_cervical_cancer_ve_on_art_received_cryotherapy_40_44_years`), every generated id equal to `generateIndicatorId`, 8 bases with data summed identically before and after, 11 run rows, 37 version rows and 0 schedules parsed with no retired key, a second run pending nothing. |
| 2026-09-11 | 4 | The migration runner's connection suppresses notices (`onnotice: () => {}` in `server/db/postgres/connection_manager.ts`, S2, outside the Surface), so the id table 086 raises never reaches the server log. `validate_indicator_sources` prints it (old raw id, label, new base) instead; ruling 12's NOTICE clause is met only through the validator. |
| 2026-09-11 | 4 | Ruling 10 in PL/pgSQL: five letters in U+00C0..U+017F decompose to TWO characters under NFKD (Ŀ ŀ Ĳ ĳ ŉ), which a one-to-one `translate` cannot express, so 086 expands them with a `replace` chain first. `server/tests/indicator_sources_migration_test.ts` emulates the SQL pipeline over every code point in the range against `slugIndicatorId`, and pins `v_special` and `v_reserved` to the lib constants. |
| 2026-09-11 | 4 | Ruling 4 additions, all rewritten by 086: the datatable toggle `IndicatorType` (`raw` / `common`) is `HmisDatatableView` (`source` / `indicator`) and the request field `rawOrCommonIndicators` is `view`; the delete window's `indicatorType` / `rawIndicatorsToInclude` are `grain` / `sourcesToInclude` (stored in deletion version rows); the CSV diagnostics block `unmappedIndicators` is `unknownSources` with `source_id` samples; `PeriodIndicatorRawStat` is `PeriodSourceStat`. The wire selection is `Dhis2RunSelectionInput` (a window carries `indicatorIds`); the stored one, `Dhis2RunSelection`, adds `sourceIds`, `populationTermsDropped` and `nonDhis2SourcesDropped` (empty lists on rows 086 migrated, which carry `indicatorIds: []`). Migrated `periodIndicatorStats` rows that older CSV code wrote under `indicatorCommonId` are renamed to `sourceId` too, and the client's fallback cast is gone. |
| 2026-09-11 | 4 | Route registry keys are the new names: `createIndicators`, `updateIndicator`, `deleteIndicators`, `reorderIndicators`, `batchUploadIndicators`; the `/indicators-raw*` and `/indicators/batch-raw` routes are gone. The SSE and instance-detail counts are `indicators.hmisIndicators` / `hmisSources`. The two stamp names `indicatorMappingsVersion` / `baseIndicatorMappingsVersion` keep "Mappings": S8's run manifest and dataset-info types carry them and are outside the Surface. |
| 2026-09-11 | 4 | `/dhis2-indicators-export` (health, unauthenticated, read outside the repo per SYSTEM_15) keeps its wire keys `id`, `label`, `mappedTo`, now served from `indicator_sources`. |
| 2026-09-11 | 4 | Gate 3's `is_default` pattern also matches `is_default_visualization` in S9/S12 project files. With a word boundary the grep is at zero outside migrations except one pre-existing comment in `server/run_query/virtual_defaults.ts` (S9, outside the Surface) that describes presentation objects. |
| 2026-09-11 | 4 | Intermediate state kept as listed: the DHIS2 select form still offers DHIS2 indicators; selecting one creates a base whose sole source is the indicator UID, which the dispatcher fails permanently (as it did the raw), until step 5's decomposition replaces the form. The editor takes a base's sources as free id + label pairs (CSV sources carry no metadata) and refuses a source another indicator owns before the server does. The ledger tab is labelled "By source" (its grain), each row showing the owning indicator; the ledger detail's action is "Re-import this source". |
| 2026-09-11 | 4 | `_main_database_types.ts` had `DBIndicatorRaw` and `DBIndicatorMapping` with no importer; deleted. `lib/mod.ts` was not touched: the selection expansion lives in `lib/common_indicator_catalog.ts`. SYSTEM_06's display-cache sentence said the HMIS items key on `indicatorMappingsVersion`; the code keys on the base stamp, corrected while rewriting that paragraph. |
| 2026-09-11 | 4 | Floor and gates: typecheck (every tracked file claimed; SYSTEM_05 and SYSTEM_06 globs gained the two harnesses), `deno task test` 74 passed (10 new across `indicator_selection_expansion_test.ts` and `indicator_sources_migration_test.ts`), `validate_protocols`, `validate_migrations` (086 a no-op on the fresh replay; 003, 056 and 079 guarded), `validate_queries`, `validate_fresh_boot` (22 specials as empty bases, no default flag), gates 1 to 3 at zero. The server booted against the dev database ("Applying migration: 086_indicator_sources.sql"), ran the in-boot suite (74 passed) and reached "Listening" on port 8000; stopped afterwards. The dev database after: 23 indicators, 15 sources, `indicators_raw`, `indicator_mappings` and `is_default` gone, every run row's selection under the new keys. |
| 2026-09-11 | 4 | Step 4 built |
| 2026-09-11 | 4 | Review: `server/db/instance/indicators.ts:767-820` and `:864-881`. In the batch upload a source the file moves between owners (its current owner also in the file, or dropped by a replace) is neither in `removedSourceIds` nor refused as owned elsewhere, so the outcome depends on row order: the old owner's row first deletes the source and, when it has data, `dataset_hmis_source_id_fkey` (RESTRICT, never deferred) aborts the write with the raw constraint error; the new owner's row first re-homes the source and its data through `writeSources`' `ON CONFLICT DO UPDATE` with no message, and a replace that drops the old owner cascades the source away before the new row is written. Fix: count a moved source as removed from its old owner in the data pre-check (refused with the listing when it has data), and write moves before deletes so a data-free move does not depend on row order. |
| 2026-09-11 | 4 | Review: `client/src/components/instance_dataset_hmis/_import_information.tsx:145`. The staging statistics section is still titled "Period-indicator combinations" (three languages) over rows the step changed to show `stat.sourceId`; the grain is period by source. Rename the heading. |
| 2026-09-11 | 4 | Review: every gate re-run green by the reviewer: typecheck (every tracked file claimed), `deno task test` 74 passed, `validate_protocols`, `validate_migrations` (89 instance migrations, schema unchanged), `validate_queries` (63 cases), `validate_fresh_boot` (22 specials as empty bases), gates 1 to 3 at zero (gate 3 with the word boundary the Do session noted). The committed tree booted against the dev database on `PORT=8001` (086 already applied, in-boot suite 74 passed) to "Listening"; stopped afterwards. The dev `main` database after: 23 indicators (4 derived), 15 sources, no data row without a source, both FKs under their schema names, 11 run selections under the new keys, `indicators_raw`, `indicator_mappings` and `is_default` gone. Diff stat against the Surface: every changed file is listed in it; `validate_indicator_sources.ts` sits beside its script as `validate_fresh_boot.ts` does. |
| 2026-09-11 | 4 | Review: gate 6 reproduced over the dev dump only (no fleet dump reaches this session either). The pre-resolution dump fails on the derived special `anc1` with the old tables, columns, `is_default` and the migration record intact; a copy resolved by hand the way §8 describes (anc1 retyped to a base, `Bl9cq5ZpeU5` kept on `anc4`) passes with the same 8 sources, 7 new bases and identical per-base sums the Do session recorded. §7's run over every instance stays the rollout precondition. 086 checks the derived special before the shared raws, so an instance carrying both sees one listing per attempt. |
| 2026-09-11 | 4 | Review: Deliverable checked in code. 086 as ruling 12 spells it (guards, mapped raws to sources, unmapped raws to bases through the PL/pgSQL generator pinned to lib by the harness, guarded column renames, FKs dropped by `confrelid` and added under the schema names, `is_default` dropped, every stored JSON shape rewritten including step 2's two optional keys, schedules re-pointed at owning bases, old tables dropped last); 003, 056 and 079 guarded. One CRUD (`createIndicators`, `updateIndicator`, `deleteIndicators`) writing sources in the indicator's transaction with the ownership, data and expression pre-checks; the ruling 11 batch file over `INDICATOR_BATCH_FILE_COLUMNS` with the download mirroring it; both stamps over `indicators` and `indicator_sources`; the extract, the datatable's two views and the windowed delete joined on `source_id`; `validateRunSelection` expanding through `expandIndicatorSelectionToSources`, the expansion stored on the row and carried in the worker message, the queued launch passing the stored selection through untouched; the CSV staging column and diagnostics renamed; the manager as one table with Type, sources in the row, the Special badge and the reference modal; the raw tab, editor and batch form gone; picker, wizard, history, run detail, by-source tab and ledger detail on indicators and sources; the select form creating one base per element through `generateIndicatorId`; cache name `instance_indicators_v4`. |
| 2026-09-11 | 4 | Review: `_tab_by_indicator.tsx` (`Dhis2TabByIndicator`, tab id `by_indicator`) and `_ledger_indicator_detail.tsx` keep their names while the tab reads "By source". The Surface and SYSTEM_06 name those files, so the rename is not this step's and waits on a ruling. No code change. |
| 2026-09-11 | 4 | Step 4 reviewed: 2 findings |
| 2026-09-11 | 4 | Fix (same session as the review, at Tim's instruction): `batchUploadIndicators` now lists the sources the file moves between owners (`moved`: file owner differs from current owner), refuses with the listing when any has data ("The file would move sources that have data to another indicator"), and applies the surviving moves as `UPDATE indicator_sources SET indicator_id` before the replace's indicator deletes and the per-row source deletes, so a data-free move lands under its new owner whatever the row order and keeps its label (labels now come from one map over every existing source). The removed-sources check is unchanged. |
| 2026-09-11 | 4 | Fix: the staging statistics heading in `_import_information.tsx` reads "Period-source combinations" (en/fr/pt). |
| 2026-09-11 | 4 | Fix proof, one-off (not a gate; ruling 11 has no harness in §5): a scratchpad harness booted a throwaway postgres through `dbStartUp`, seeded two bases with one source and a data row, and called `batchUploadIndicators` six ways (upsert with the old owner's row first, upsert with the new owner's row first, replace dropping the old owner; each with and without data). With data all three are refused with the listing and nothing changes; without data all three move the source with its label kept. Floor re-run green: typecheck, `deno task test` 74 passed, `validate_protocols`, boot to "Listening" on `PORT=8001`. |
| 2026-09-11 | 4 | Step 4 fixed |
| 2026-09-11 | 4 | Review (of fix 4): `server/db/instance/indicators.ts:855-864`. The move loop runs before the per-row `INSERT INTO indicators ... ON CONFLICT` (line 883), and `indicator_sources.indicator_id` is a plain (non-deferrable) FK, so a data-free move to an indicator the file creates fails inside the transaction with the raw `indicator_sources_indicator_id_fkey` error (code 23503) whatever the row order, and with a replace that drops the old owner too. Reproduced on a throwaway postgres through `dbStartUp` with `src1` under `anc1` and a file listing it under a new `newbase`: three variants refused with the constraint error and nothing changed; the same move to the existing `anc4` lands with its label kept, and every variant with data is refused with the listing. Fix: upsert the file's indicator rows before the moves (or apply a move only after its new owner's row exists), keeping the UPDATE rather than a delete-and-reinsert, since the ledger CASCADEs on `source_id` and a source with no data can still carry ledger rows. |
| 2026-09-11 | 4 | Review (of fix 4): gate 2's pattern has no migrations exclusion, and 086 names `computedIndicators` twice (a comment and the jsonb strip of the retired key at line 428), which the JSON rewrite of ruling 12 requires. With the `server/db/migrations/**` exclusion gate 3 carries, gates 1 to 3 are at zero. Plan wording only; no code change. |
| 2026-09-11 | 4 | Review (of fix 4): the two review 4 findings are closed in code: `batchUploadIndicators` lists `moved` (file owner differs from current owner), refuses with the data listing, and applies the surviving moves first; `_import_information.tsx:146` reads "Period-source combinations" in three languages. Diff stat since the review commit: both changed files are in the Surface. Every gate re-run green: typecheck (every tracked file claimed), `deno task test` 74 passed, `validate_protocols`, `validate_migrations` (schema unchanged), `validate_queries`, `validate_fresh_boot` (22 specials as empty bases), gates 1 to 3 at zero as the row above qualifies. Gate 6 over the dev dump: the pre-resolution dump fails on the derived special `anc1` with nothing else changed; a copy resolved by hand (anc1 retyped to a base, `Bl9cq5ZpeU5` kept on `anc4`) passes with 8 sources, 7 new bases, 8 bases with identical sums, 11 run rows, 37 version rows, 0 schedules. No fleet dump reaches this session; §7's run stays the rollout precondition. The committed tree booted against the dev database on `PORT=8001` (in-boot suite 74 passed) to "Listening"; stopped afterwards. |
| 2026-09-11 | 4 | Step 4 reviewed: 1 finding |
| 2026-09-11 | 4 | Fix (same session as the review, at Tim's instruction): the batch transaction now runs in the order indicator upserts, source moves, replace deletes, per-row source writes, expression check. A moved source's new owner therefore exists before the UPDATE, and a replace that drops the old owner no longer cascades the moved source (and its ledger rows) away before the move. Proof, one-off on a throwaway postgres through `dbStartUp` with a ledger row on the source: the four data-free variants (to an existing base; to a new base with the old owner's row first, the new owner's row first, and a replace dropping the old owner) all land under the new owner with the label and the ledger row kept; the four with data are refused with the listing and nothing changes. Floor re-run green: typecheck, `deno task test` 74 passed, `validate_protocols`, boot to "Listening" on `PORT=8001` (in-boot suite 74 passed). |
| 2026-09-11 | 4 | Step 4 fixed |
| 2026-09-11 | 4 | Review (of fix 4, second): the finding is closed in code. `batchUploadIndicators` (`server/db/instance/indicators.ts:855-905`) runs the transaction as indicator upserts, source moves, the replace's indicator deletes, per-row source deletes and writes, then the expression check, so a moved source's new owner exists before the `UPDATE` and a dropped old owner cannot cascade the moved source away. Reproduced on a throwaway postgres through `dbStartUp` with `src1` under `anc1` carrying a ledger row: the four data-free variants (to the existing `anc4`; to a new base with the old owner's row first, the new owner's row first, and a replace dropping the old owner) land under the new owner with the label and the ledger row kept; the four with data are refused with "The file would move sources that have data to another indicator: src1 (1 records)" and nothing changes. |
| 2026-09-11 | 4 | Review (of fix 4, second): every gate re-run green by the reviewer: typecheck (861 tracked files claimed), `deno task test` 74 passed, `validate_protocols`, `validate_migrations` (89 instance and 43 project migrations, schema unchanged), `validate_queries` (63 cases; its first run failed loading the postgres dependency while four container gates ran concurrently and passed alone, so run it by itself), `validate_fresh_boot` (22 specials as empty bases), gates 1 to 3 at zero with the migrations exclusion and the `is_default` word boundary. Gate 6 over the dev dump: the pre-resolution dump fails on the derived special `anc1` with nothing else changed; a copy resolved by hand (`anc1` retyped to a base, `Bl9cq5ZpeU5` kept on `anc4`) passes with 8 sources, 7 new bases, 8 bases with identical sums, 11 run rows, 37 version rows, 0 schedules. No fleet dump reaches this session; §7's run stays the rollout precondition. The committed tree booted against the dev database on `PORT=8001` (in-boot suite 74 passed) to "Listening"; stopped afterwards. Diff stat since the review commit: both changed files are in the Surface. |
| 2026-09-11 | 4 | Step 4 reviewed: pass |
| 2026-09-11 | 5 | `lib/types/indicators.ts` (outside the Surface) gained the naming wire types (`IndicatorNamingTarget`, `IndicatorNamingSource`, `IndicatorNamingDerived`, `IndicatorNamingInput`) and the two refusal describers (`describeDhis2SourceRefusal`, `describeDhis2ParseRefusal`, three-language, used by the server's error text and the client's list). The route registry files are not re-exported by the lib barrel, so a type shared by client and server has no other home; step 3's wire types set the precedent. |
| 2026-09-11 | 5 | The naming step is saved by one DB function, `applyIndicatorNaming`: new bases grouped by chosen id (several candidates under one new id become one base, labelled by the first), sources added to existing bases (a source already under its target is nothing to write; a `new` target under an existing id is refused so the user picks "Add to an existing indicator"), and derived indicators with their expressions rewritten from source ids to base ids through `renameIdentifiers`, all under `createIndicators`' pre-checks, which moved into `checkIndicatorWrites` + `insertIndicators` for the purpose (behaviour unchanged). `createIndicatorsFromDhis2` wraps it behind verdicts and decompositions; the `/indicators-dhis2/create` route re-reads every element and indicator from DHIS2 and judges them itself, never the client's search results. |
| 2026-09-11 | 5 | The CSV path has no naming route of its own: `resolveDatasetHmisCsvReview`'s body gained the action `restage` with an optional `naming`. The route applies the naming (`applyIndicatorNaming`) and notifies indicators-updated before calling the resolver for the relaunch, so a refused naming leaves the hold untouched and a relaunch refused afterwards (the asset's pin no longer matches) cannot hide indicators that exist from the clients (found by Tim after the build; first cut applied the naming inside the resolver and notified only on the relaunch's success). `worker.ts` needed no change: it stores the staging result whole, and the stager now records `unknownSources.ids`. |
| 2026-09-11 | 5 | `unknownSources.ids` is optional on the stored JSON: results staged before this step lack it (version rows, complete run rows, and any hold spanning the deploy), and no migration backfills it. The card offers the re-stage action only when the set is present and non-empty; older results still render. |
| 2026-09-11 | 5 | Operand labels in the select form: an operand of a selected DHIS2 indicator whose element the search did not return is looked up by id through the existing `searchDhis2All` route (one extra request at Next), its COC name from the element's category combo; the id is the fallback label. Source labels are not editable in the naming step (they are DHIS2's names, or the CSV id). |
| 2026-09-11 | 5 | Harness: `server/tests/indicator_naming_test.ts` (14 cases) builds a throwaway `naming_test_<timestamp>` database from `_main_database.sql` on the dev postgres (the `.env` the test task loads), drops stale ones at start and its own at the end, so it runs at every dev boot like the rest of the suite. Cases: a refused element, a refused operand, a refused formula, an unnamed operand and a taken derived id each create nothing; a decomposed indicator creates its bases, sources and derived with `(anc4 / (anc1 + anc1))`; operands attached to existing bases rename the expression to them; attach semantics (adds, no-op when already owned, refused for a derived, a missing indicator or a source owned elsewhere); reserved and special ids; a derived naming an unlisted source. |
| 2026-09-11 | 5 | Re-stage exercised against the dev database, run 60: a CSV of 78 rows (three facilities, two months, one known source `zhxRZPwaIfT` and twelve unknown ids `csv_test_source_01..12`) launched through `launchDatasetHmisCsvRun` held in `needs_review` with `unknownSources.total` 12, the sample capped at 10 and `ids` carrying all 12. `resolveDatasetHmisCsvReview` with `restage` and a naming of ten new bases, two ids grouped under one new base (`csv_test_pair`) and one attached to the existing empty base `measles2` created them and relaunched the run through the stage leg (progress `staging`, `csv_config` without `resumeFromStaging`); it integrated as version 38 with 78 rows across 13 sources and 0 unknown sources at completion. |
| 2026-09-11 | 5 | The exercise's artefacts were then removed by SQL so the dev dictionary is as it was (23 indicators, 15 sources): the eleven test indicators (their sources and ledger rows cascading), the source attached to `measles2`, the 72 test data rows, and version 38 with run 60's `version_id` nulled; run 60 stays as `complete`. The merge had also overwritten four existing `penta1` rows at two facilities and inserted two at a third: the four were restored from `~/wb-fastr-dev-main-before-086.sql.gz` (108, 31, 121, 33; version 37), the two deleted, and the two ledger pair rows recomputed. The test CSV was deleted from the assets folder. |
| 2026-09-11 | 5 | Fact now wrong outside the Surface: SYSTEM_07's "The dictionary client does not read the verdict or the decomposition yet (PLAN_A3 step 5)" (search-result shaping paragraph). Step 6's read-through rewrites it. |
| 2026-09-11 | 5 | Floor and gates: typecheck (every tracked file claimed; SYSTEM_05 globs gained the harness), `deno task test` 88 passed (14 new), `validate_protocols` (0 new flags), gates 1 to 3 unchanged. The server booted against the dev database on `PORT=8001` (no migration to apply; in-boot suite 88 passed) and reached "Listening"; stopped afterwards. `validate_migrations` and `validate_queries` not run: the step touches neither. |
| 2026-09-11 | 5 | Step 5 built |
| 2026-09-11 | 5 | Review: `server/routes/instance/datasets.ts:465` fires `notifyInstanceIndicatorsUpdated` only when `resolveDatasetHmisCsvReview` returns success, but on `restage` the naming is committed before the relaunch, and a relaunch that fails after it (`spawnCsvRunWorker` throwing on a changed asset pin or a missing file, the refusal ruling 6 designs for) returns `success: false` through `tryCatchDatabaseAsync`. The indicators exist, the run is `error`, and no `indicators_updated` reaches the clients, so the dictionary cache (`instance_indicators_v4`, keyed on the SSE summary's version) shows the old list until another indicator write or a reload. Fix: notify whenever a naming was given and applied, independent of the relaunch's outcome (either notify on `body.naming` regardless of `res.success`, a refused naming costing one harmless push, or have the DB function report that the naming landed). Code change. |
| 2026-09-11 | 5 | Review: every gate re-run green by the reviewer: typecheck (every tracked file claimed), `deno task test` 88 passed (the naming harness builds and drops its throwaway database), `validate_protocols` (0 new flags), gates 1 to 3 at zero (gate 2's two hits are 086's own key-stripping SQL, gate 3 with the migrations exclusion and the `is_default` word boundary). The committed tree booted against the dev database on `PORT=8001` (no migration to apply, in-boot suite 88 passed) to "Listening"; stopped afterwards. Run 60 checked in the dev `main` database: `complete`, `csv_config` without `resumeFromStaging`, its final staging result 78 rows and 0 unknown sources, `version_id` null; dictionary at 23 indicators and 15 sources, no `csv_test*` row, `measles2` with no source, version 38 gone. Diff stat against the Surface: every changed file is listed in it except `lib/types/indicators.ts`, the deviation the Do session recorded. Deliverable checked in code: `_naming_step.tsx` is the one component both hosts render, proposing ids through `generateIndicatorId` against the dictionary plus earlier proposals, editable inline or "Add to an existing indicator", an already-owned source pinned to its owner, `namingIssues` mirroring the server's refusals; the select form shows each element's and indicator's refusal from S7's verdict and decomposition and disables Add, then names the selection with every operand as a candidate; `/indicators-dhis2/create` re-fetches elements and indicators by id, judges them with `getDhis2OperandVerdict` and `withDecompositions`, and `createIndicatorsFromDhis2` refuses the whole save on any refusal before `applyIndicatorNaming` writes bases, attachments and the rewritten derived in one `begin`; the stager records `unknownSources.ids` as the full distinct set, the card offers the third action when it is present, and `restage` strips `resumeFromStaging` so `spawnCsvRunWorker` re-resolves the asset against its pin and `stageHmisCsvIntoTables` pre-drops the held table. The resolve-review route carries `can_configure_data`, the same permission as the indicator create routes. |
