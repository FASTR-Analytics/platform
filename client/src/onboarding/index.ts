...
//
// A tab's tours are split into parts with independent seen-flags: a viewer
// part runs for everyone, an editor part is permission-gated, and card parts
// are deferred until the project has content. Parts eligible at the same
// moment merge into one seamless run in this array order; a part whose
// condition only holds later runs on the first visit where it does. The
// editor overlays render on top of the still-mounted project shell, so
// projectTab() stays put inside them: the AI view is what actually tracks
// where the user is. Tab pages must exclude the editing views, or a list tour
// could fire behind the editor.
