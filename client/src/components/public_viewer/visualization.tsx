...
export default function PublicVisualization() {
  ...
  const download = async () => {
    ...
    if (!canvas) return;
    const pad = 50;
    const backCanvas = new OffscreenCanvas(
      canvas.width + 2 * pad,
      canvas.height + 2 * pad,
    );
  ...
  return (
    ...
    >
      <Switch>
        ...
        <Match
          when={
            viewState().status === "password_required" ||
          ...
          {(() => {
            ...
            return (
              <PasswordGate
                ...
                wrongPassword={
                  (state as { wrongPassword?: true }).wrongPassword
                }
                ...
                {state.status === "ready" &&
                  (() => {
                    const fi = hydrateFigureInputsForPublicRendering(
                      (state as { bundle: ShareVizBundle }).bundle
                        .strippedFigureInputs,
