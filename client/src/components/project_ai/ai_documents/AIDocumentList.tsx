import { Icon } from "panther";
import { Show, For } from "solid-js";

export function AIDocumentList(p: {
  sent: { title: string }[];
  pending: string[];
  onRemovePending: (assetFilename: string) => void;
}) {
  return (
    <Show when={p.sent.length > 0 || p.pending.length > 0}>
      <div class="flex flex-wrap gap-1 px-2 py-2 border-b">
        <For each={p.sent}>
          {(doc) => (
            <div class="flex items-center gap-1 bg-base-200 rounded px-2 py-1 text-xs">
              <Icon iconName="file" class="w-3 h-3 text-primary" />
              <span class="max-w-[150px] truncate">{doc.title}</span>
            </div>
          )}
        </For>
        <For each={p.pending}>
          {(filename) => (
            <div class="flex items-center gap-1 bg-base-200 rounded px-2 py-1 text-xs">
              <Icon iconName="file" class="w-3 h-3 text-primary" />
              <span class="max-w-[150px] truncate">{filename}</span>
              <button
                type="button"
                class="cursor-pointer"
                onClick={() => p.onRemovePending(filename)}
              >
                <Icon iconName="x" class="w-3 h-3" />
              </button>
            </div>
          )}
        </For>
      </div>
    </Show>
  );
}
