import { AIChatProvider, type AIChatConfig, FrameRight, FrameRightResizable } from "panther";
...
import { useAIDocuments } from "~/components/ai_documents";
...
function AIProjectWrapperInner(props: AIProjectWrapperProps) {
  ...
  // Single project-level conversation - aiContext only changes tools/prompts
  ...
  // Subscribe to SSE changes with context-aware filtering
  onMount(() => {
    const cleanup = addListener((tableName, ids, timestamp) => {
      const ctx = aiContext();

      // Slides - only notify if relevant to current deck
      if (tableName === "slides") {
        if (ctx.mode === "deck") {
          // Only notify about slides in current deck
          const deckSlides = ctx.getSlideIds();
          const relevantIds = ids.filter(id => deckSlides.includes(id));
