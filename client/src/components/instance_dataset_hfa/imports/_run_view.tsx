...
// Running HFA run: staging/integrating percentage (relocated from the deleted
// attempt wizard's progress views).
export function HfaRunView(p: Props) {
  ...
  async function attemptCancel() {
    const cancelAction = createDeleteAction(
      ...
      () => serverActions.cancelDatasetHfaRun({ runId: p.run.id }),
      p.onChanged,
    );
    ...
    >
      <div class="ui-pad ui-spy">
        <div class="ui-gap flex items-baseline">
          ...
          <div class="text-sm">
            {phaseLabel()} <span class="font-mono">{p.run.csvFileName}</span>
          </div>
        </div>
