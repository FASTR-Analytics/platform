...
export function MoveToFolderModal(p: AlertComponentProps<Props, ReturnType>) {
  ...
  const header = p.presentationObjectIds.length > 1
    ? `${t3({ en: "Move", fr: "Déplacer", pt: "Mover" })} ${p.presentationObjectIds.length} ${t3({ en: "visualizations to folder", fr: "visualisations vers le dossier", pt: "visualizações para a pasta" })}`
