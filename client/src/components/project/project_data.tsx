...
export function ProjectData(p: Props) {
 ...
 class=""ensureHeightAsIfButton></HeadingBar>}>
 ...
 reasons.push(t3({ en:`Dataset updated (v${proj} → v${inst})`, fr:`Données mises à jour (v${proj} → v${inst})`, pt:`Conjunto de dados atualizado (v${proj} → v${inst})`}));
                  }
 ...
 reasons.push(t3({ en:"Facilities or admin areas changed", fr:"Établissements ou unités administratives modifiés", pt:"Estabelecimentos de saúde ou zonas administrativas alterados"}));
                  }
 ...
 reasons.push(t3({ en:"Indicators or mappings changed", fr:"Indicateurs ou correspondances modifiés", pt:"Indicadores ou correspondências alterados"}));
                  }
 ...
 reasons.push(t3({ en:"Facility configuration changed", fr:"Configuration des établissements modifiée", pt:"Configuração dos estabelecimentos de saúde alterada"}));
                  }
 ...
 reasons.push(t3({ en:"Admin area structure changed", fr:"Structure des unités administratives modifiée", pt:"Estrutura das zonas administrativas alterada"}));
                  }
 ...
 reasons.push(t3({ en:"Calculated indicators changed", fr:"Indicateurs calculés modifiés", pt:"Indicadores calculados alterados"}));
                  }
 ...
 outline
                          >
                            {t3({ en:"Disable", fr:"Désactiver", pt:"Desativar"})}
                          </Button>
                          <Button
                    ...
                    <div class="ui-pad ui-spy-sm">
                      <Show when={isStale()}>
                        <div class="ui-spy-sm mb-4 inline-block ui-pad border rounded">
                          <div class="font-700">
                            {t3({ en:"Project data is out of date", fr:"Les données du projet ne sont plus à jour", pt:"Os dados do projeto estão desatualizados"})}
                          </div>
 ...
 iconName="refresh"
                            >
                              {t3({ en:"Update data", fr:"Mettre à jour les données", pt:"Atualizar os dados"})}
                            </Button>
                          </div>
                        </div>
                      </Show>
                      <div class={`text-xs ${isStale() ?"text-warning":"text-success"}`}>
                        {t3({ en:"Last exported from instance into project", fr:"Dernière exportation de l'instance vers le projet", pt:"Última exportação da instância para o projeto"})}:{""}
                        {new Date(
...
"year-month",
 getCalendar(),
                        ...
                        <div class="grid grid-cols-12 text-sm">
                          ...
                          <div class="col-span-8">
                            {keyedProjectDatasetHmis.info.windowing
                              .takeAllIndicators
                              ? t3({ en:"All indicators", fr:"Tous les indicateurs", pt:"Todos os indicadores"})
 ...
 return Array.from(grouped.entries())
                              ...
                              : !keyedProjectDatasetHmis.info.windowing.takeAllAdminArea2s &&
                                ...
                                : t3({ en:"All admin areas", fr:"Toutes les unités administratives", pt:"Todas as zonas administrativas"})}
                          </div>
                        </div>
                        <Show
 when={
 instanceState.facilityColumns
                        ...
                        >
                          <div class="grid grid-cols-12 text-sm">
                            <div class="col-span-4">
                              {t3({ en:"Facility ownership categories", fr:"Catégories de propriété des établissements", pt:"Categorias de propriedade dos estabelecimentos de saúde"})}
                            </div>
                            <div class="col-span-8">
                              {keyedProjectDatasetHmis.info.windowing
                                .takeAllFacilityOwnerships !== false
                                ? t3({ en:"All facility ownership categories", fr:"Toutes les catégories de propriété", pt:"Todas as categorias de propriedade dos estabelecimentos de saúde"})
...
",",
 ...
 instanceState.facilityColumns.includeTypes
                        ...
                        >
                          <div class="grid grid-cols-12 text-sm">
                            <div class="col-span-4">
                              {t3({ en:"Facility types", fr:"Types d'établissements", pt:"Tipos de estabelecimentos de saúde"})}
                            </div>
                            <div class="col-span-8">
                              {keyedProjectDatasetHmis.info.windowing
                                .takeAllFacilityTypes !== false
                                ? t3({ en:"All facility types", fr:"Tous les types d'établissements", pt:"Todos os tipos de estabelecimentos de saúde"})
...
",",
                      ...
                      <Show when={keyedProjectDatasetHmis.info.totalRows}>
                        <div class="font-700 grid grid-cols-12 text-sm">
                          <div class="col-span-4">
                            {t3({ en:"Total rows", fr:"Nombre total de lignes", pt:"Total de linhas"})}
                          </div>
                          <div class="col-span-8">
                            {toNum0(
 ...
 fallback={
                              <span class="text-warning">
                                {t3({ en:"Not snapshotted (re-export to include)", fr:"Non capturé (réexporter pour inclure)", pt:"Não capturado (reexportar para incluir)"})}
                              </span>
                            }
                          >
                            <span class={instanceState.calculatedIndicatorsVersion === keyedProjectDatasetHmis.info.calculatedIndicatorsVersion ?"text-success":"text-warning"}>
                              {instanceState.calculatedIndicatorsVersion === keyedProjectDatasetHmis.info.calculatedIndicatorsVersion
                                ...
                                : t3({ en:"Out of date", fr:"Obsolète", pt:"Desatualizado"})}
                            </span>
                          </Show>
                        </div>
                      </div>
 ...
 await openAlert({
 text: t3({ en:"This dataset has no data at the instance level", fr:"Ce jeu de données ne contient aucune donnée au niveau de l'instance", pt:"Este conjunto de dados não contém dados ao nível da instância"}),
 intent:'danger'
                    });
 return;
                  }
 ...
 label: { en:"HFA dataset updated", fr:"Données HFA mises à jour", pt:"Conjunto de dados HFA atualizado"},
                    },
                    {
 instance: instanceState.hfaIndicatorsVersion,
 project: info.hfaIndicatorsVersion,
 label: { en:"HFA indicators changed", fr:"Indicateurs HFA modifiés", pt:"Indicadores HFA alterados"},
                    },
                    {
 ...
 return {
 isStale: true,
 reasons: [t3({ en:"ICEH data updated", fr:"Données ICEH mises à jour", pt:"Dados ICEH atualizados"})],
                    };
                  }
 ...
 return {
 success: false,
 err: t3({ en:"This dataset has no data at the instance level", fr:"Ce jeu de données ne contient aucune donnée au niveau de l'instance", pt:"Este conjunto de dados não contém dados ao nível da instância"}),
                    };
                  }
