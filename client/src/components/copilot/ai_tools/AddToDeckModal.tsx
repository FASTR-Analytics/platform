import { productScope, t3, type Slide } from "lib";
...
export function AddToDeckModal(p: AlertComponentProps<Props, ReturnType>) {
  ...
  const save = createFormAction(
    async (e: MouseEvent) => {
      ...
      // The draft was resolved under the copilot's pair at draft time: the pin
      // at national scope, since this modal only opens when NO deck editor is
      // up. The target deck may sit on another package or another admin area,
      // so its figures are re-queried under the deck's own pair before the
      // write (D15).
      ...
      if (!target) {
        return {
          ...
          err: "That slide deck no longer exists",
        };
      }
      let slide: Slide;
      try {
        ...
        return {
          ...
          err: err instanceof Error
            ? err.message
            : "Could not prepare the slide for this deck",
        };
      }
