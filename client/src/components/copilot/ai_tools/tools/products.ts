...
// package (PLAN_PRODUCTS_RESTRUCTURE D3/D16). The lists come from instance T1
// at call time, so they are never frozen at tool construction and there is no
// list route to call. `create_report` is the copilot's one non-editor write,
// approval-gated.
    ...
    createAITool({
      ...
      approval: {
        propose: (input) => ({
          ...
          commit: async () => {
            const createRes = await createLabelledProduct(
              "report",
              input.label,
            );
