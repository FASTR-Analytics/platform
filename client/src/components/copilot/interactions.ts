...
// SSE side-channel (index.tsx) and the editors and selection UIs. The engine
// owns the transactional drain at turn creation and the reduction pipeline
...
export const copilotInteractions = defineAIInteractions({
  ...
  // reattach or scope change; a report's on every body save. Reported in every
  // view (a product the user is not looking at can still change under a
  ...
  // never learn its draft was accepted, or worse would see a generic
  // "deck changed" line misattributed to a collaborator.
