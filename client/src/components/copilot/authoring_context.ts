...
// ============================================================================
// The copilot's live (package, scope) binding and its authoring context
// ============================================================================
//
// The env's pair is the OPEN product's while an `editing_*` view is active,
// else the instance pin at national scope (PLAN_PRODUCTS_RESTRUCTURE D15).
// Both halves move mid-conversation, and the tools array is built exactly ONCE
// at wrapper setup: panther registers `config.tools` into its ToolRegistry at
// chat construction and never re-reads the array. So the tool-aliasing
// invariant (SYSTEM_13, "Tool freshness rests on store aliasing, not
// reactivity") is what keeps the copilot pointed at the right package: this
// store is RECONCILED IN PLACE, so `metrics`, `icehIndicators` and
// `hfaTaxonomy` keep their object identity across a package switch and the
// arrays captured by the shared tool factories stay live. Replacing any of
// these fields with a fresh array or object instead of reconciling would
// silently freeze the AI's world with no error, the same failure the /mcp host
// avoids by hydrating its captured arrays in place.
//
// `scope` is read through `requireCopilotScope()` at CALL time rather than
// captured, because a handler that started under one product may finish under
// another.
...
// The ONE place a tool handler learns which package and scope it is serving.
...
// Label and generation timestamp of the package the env is currently bound to,
...
//
//  1. the pair plus the package's authoring context (immutable per runId, so
//     the T2 cache answers instantly on every revisit);
//  2. the HFA survey rounds, which are instance-wide T1 and therefore NOT in
//     the per-run payload (`RunAuthoringContext`'s seam), so an import that
//     adds a round reaches the copilot without refetching every package.
//
...
export function mountCopilotAuthoringContext(): void {
  ...
  createEffect(() => {
    ...
    setCopilotAuthoringContext(
      "scope",
      scope === null ? null : reconcile(scope),
    );
    if (scope === null) {
      applyEmptyPackage();
      return;
