/**
 * TimCacheD - IndexedDB-Backed Cache with LRU
 *
 * ARCHIVED: This is the original cache implementation without PDS integration.
 * Kept here for reference and potential reuse in other projects that don't have
 * ProjectDirtyStates infrastructure.
 *
 * Features:
 * - Two-tier caching: Memory (LRU) + IndexedDB (persistent)
 * - Version-based cache invalidation
 * - Handles in-flight promises (deduplication)
 * - Manual version parameter passing
 * - Supports "any_version" mode
 * - LRU eviction for memory layer
 *
 * Usage example:
 * ```typescript
 * import { get, set } from "idb-keyval";
 *
 * const cache = new TimCacheD<
 *   { id: string },           // Uniqueness params
 *   { version: string },      // Version params
 *   MyDataType                // Data type
 * >(
 *   "my-cache",               // Name (for IDB key prefix)
 *   {
 *     uniquenessHashFromParams: (p) => p.id,
 *     versionHashFromParams: (v) => v.version,
 *     parseData: (data) => ({
 *       shouldStore: data.success === true,
 *       uniquenessHash: data.id,
 *       versionHash: data.version,
 *     }),
 *   },
 *   100  // Max memory cache size (optional, default 100)
 * );
 *
 * // Usage
 * const existing = await cache.get({ id: "123" }, { version: "v1" });
 * if (!existing) {
 *   const promise = fetchData();
 *   cache.setPromise(promise, { id: "123" }, { version: "v1" });
 * }
 * ```
 *
 * NOTE: Requires idb-keyval library for IndexedDB operations.
 * Install with: npm install idb-keyval
 */
...
export class TimCacheD<UniquenessParams, VersionParams, T> {
  ...
  async setPromise(
    ...
    try {
      ...
      if (!d.shouldStore) {
        // Clean up unresolved to prevent memory leak
