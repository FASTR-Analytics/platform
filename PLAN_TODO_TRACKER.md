...
- [~] 🔥 **#1 — Nigeria AOP state decks: AI slide creation MOSTLY UNBLOCKED** — Claire, Jun 16 (direct). Project **"Nigeria General Project"**, folder **"state evidence packs"**; country prompt *"Annual Operating Plan (AOP) performance review slide deck by State"*. AI builds deck structure fine. Originally **every chart/timeseries slide failed to write**. Tied to Rachel's 36+1-state evidence-pack push (this week). **Status (Jun 17): chart/timeseries slides ✅ fixed & deployed (a); DQ-table slide (b) suspected resolved — can't replicate, confirming with Claire. Likely fully unblocked, pending her confirmation.** Two distinct errors:
  ...
  - **Likely cause (to confirm):** Zod boundary-validation mismatch in the AI slide-creation tool path (smells like the ~Jun 12–13 hardening — a too-strict `z.string()` rejecting real stored data; cf. the nanoid `z.uuid()` lesson). See PROTOCOL_APP_AI_TOOLS.md. Claire shipped 5 placeholder decks (ABIA/ANAMBRA/EBONYI/ENUGU/IMO) as a stopgap.
...
- [~] 🔥 **GeoJSON direct-import freeze — FIXED for AA2/AA3 (deploy + comms pending)** ↔ — platform froze importing GeoJSON at **admin-3 level**. Reported for **Cameroon** (May 14) and **DRC** (May 15). **Angelica: "not urgent."**
  - **Root cause (measured, Cameroon AA3):** analyze fetched the full per-level geojson (**20.4 MB / 200 district polygons, 13–43 s**) inside a 60 s budget → timeout. A geometry-payload cliff, not feature count.
  - **FIXED 2026-07-06 (`805f6b15` + `e3cac93d` + `14790e39`):** analyze is metadata-only (1–2 s, live-verified on both instances); full geometry fetched at save (180 s cap, no retry amplification); empty-map/match-prop guards; save shows matched/unmatched counts. The near-term plan is retired; **AA4** (DRC has 10,325 aires → background worker + step_3 virtualization) lives in [PLAN_GEOJSON_SNAPSHOT.md](PLAN_GEOJSON_SNAPSHOT.md) WS-EFFICIENCY. **Remaining actions: deploy + comms note to Angelica** (AA3 fixed; AA4 follows; import runs one level per run).
