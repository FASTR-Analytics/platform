...
export function parseGitHubUrl(url: string): ParsedGitHubUrl {
  ...
  if (parsed.hostname !== "github.com") {
    throw new Error(`Invalid GitHub URL: expected github.com, got ${parsed.hostname}`);
  }

  const pathParts = parsed.pathname.split("/").filter((p) => p !== "");

  if (pathParts.length < 2) {
    throw new Error(`Invalid GitHub URL: expected at least owner/repo, got ${parsed.pathname}`);
  }
...
export function buildRawGitHubUrl(
  ...
  const basePath = parsed.docsPath ? `${parsed.docsPath}/${filePath}` : filePath;
