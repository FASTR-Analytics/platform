...
import type { DocsManifest, GitHubTreeResponse, ParsedGitHubUrl } from "./types.ts";
...
export async function fetchGitHubManifest(
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch GitHub tree: ${response.status} ${response.statusText}`);
  }
