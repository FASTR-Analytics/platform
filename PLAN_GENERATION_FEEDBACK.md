...
| 2026-09-24 | 2 | Choice the plan did not cover: `runProgressSteps` clamps a `module` stage whose id is not in `moduleOrder` to `done = 2` (`indexOf` of -1 clamped to 0); the table as written would give 1, below `resolving`. |
| 2026-09-24 | 2 | Choice the plan did not cover: `buildRunPackageIntoTmp` reports `finalizing {moduleId}` only for a module that declares results objects. A module with none has no parquet to build, so "Building parquet for" would be a lie for the time it takes to skip it. |
| 2026-09-24 | 2 | Deviation: ruling 12 says `moduleFamily` becomes a reference to `datasetTypeSchema`; it had no importer outside `_module_definition_installed.ts`, so the export is deleted and its two uses name `datasetTypeSchema` directly. An alias with one file of consumers is cruft. |
| 2026-09-24 | 2 | Deviation forced by "docs move with the code": `SYSTEM_02_persistence.md` (outside the surface) lists the transform order and describes each transform; its one paragraph now names `runs_progress`. The new test file `server/tests/run_progress_test.ts` also needed an S8 manifest glob, which the deliverable named only for `lib/run_progress.ts`. |
| 2026-09-24 | 2 | Deviation: `./run` was not run. It replaces the machine-global `pg` and `valkey-local` containers, which were up and serving another session. Substituted, as Tim ruled for PLAN_COMPONENTS_TREE: `cd client && npm run build` (green) and a harness that ran `migrateRunsProgress` against the dev database inside a rolled-back transaction: 8 rows lacked a stage, all 8 transformed and validate with `stage.kind === "ended"`, a second pass transformed 0, and no row carried a stage after the rollback. |
| 2026-09-24 | 2 | Gates green at `cd9d2a91`: `deno task typecheck`, `deno task test` (464 passed, 3 ignored), `./validate_protocols` (the one stale baseline entry predates the plan), `./validate_migrations`, and `deno test -A --env-file server/tests/run_progress_test.ts` alone (4 passed). Commits `48b22aed` (schema, transform, helper, test) and `cd9d2a91` (pushes, capture cleanup), each green on its own. |
| 2026-09-24 | 2 | Step 2 built. |
| 2026-09-24 | 2 | Finding, prose only: `SYSTEM_08_results_packages.md` line 819 says a twelve-module run makes "about thirty row updates", restating §2's "about 30". Counted in the code: each module iteration pushes three times (the `module` stage, the reused/running status, the done push), `buildRunPackageIntoTmp` pushes once per module with results objects, and the fixed boundaries add about ten, so a twelve-module, three-family run makes about sixty. Thirty is the count of stage pushes alone. The design is as ruling 10 requires and no code changes; the line is corrected in step 3, whose surface holds `SYSTEM_08`. |
| 2026-09-24 | 2 | Reviewer's reading. The range since `b6a44dd9` holds only the step's three commits. Surface: `SYSTEM_02_persistence.md` is the one file outside it; it lists the transform order and describes each transform, so "docs move with the code" required its paragraph, accepted. The S8 manifest glob for `server/tests/run_progress_test.ts` is inside the surface and required by `lint:systems`, accepted. The `moduleFamily` deletion goes one step past ruling 12; no importer remains anywhere in `server/`, `lib/` or `client/src/`, and the two uses name `datasetTypeSchema`, accepted. Deliverable read in the code: `datasetTypeSchema` and `DatasetType` as `z.infer`; `_module_definition_installed.ts` references it for `family` and `dataSourceDataset.datasetType`; `_module_definition_github.ts` untouched; `runStageSchema` is §2's nine kinds with §2's payloads and `runProgressSchema.stage` is required with no `.catch` or `.optional`; every `RunProgress` literal (`launch.ts`, `pipeline.ts`, the `markRunGenerationFailed` fallback, the test) carries `stage`; the pushes sit at exactly the §2 boundaries (`exporting` before each capture, `converting` before each parquet write, `resolving`, `planning`, `module` at the top of each iteration before input hashing and the reuse lookup, `finalizing null` with `currentModuleId` cleared, `finalizing {moduleId}` before each module's results objects, `publishing` before the rename, `ended` stored by `publishReadyRun`) and nowhere else: no push per R line or per results object. The finalize choice is justified: a module with no results objects does nothing in that loop. `markRunGenerationFailed` never touches `stage`. `onProgress` is gone from the three captures and `server/runs` has no reference left; the pipeline is the only caller of both `onStage` parameters. The transform is registered right after `runs_summary`, gated on a raw `"stage"` scan, skips and logs an unparsable or non-validating row, and is idempotent: proven by a rolled-back harness against the dev database that added a legacy-shaped row, an unparsable row, a wrong-shape row and a row already carrying `publishing`: 9 of 12 transformed (the 8 dev rows plus the legacy row), the two bad rows logged and left as found, the staged row untouched, second pass 0, nothing left after rollback. `lib/run_progress.ts` is exported from `lib/mod.ts`, claimed by S8, imports only lib modules (`t3` reaches panther through the existing `lib/translate` re-export), authors en, fr and pt for every sentence, names families through `getModuleFamilyLabel` and modules through the registry; the test pins done/total for all nine kinds plus the pre-marked reuse case. No client file changed. `PROTOCOL_APP_MIGRATIONS.md` names `runs_progress` in the listing and the order. No em-dash added. Gates run here at `d466c0dc`: `deno task typecheck` (server, client, `lint:systems`, `lint:structure`, `lint:text-sizes` all OK), `deno task test` 464 passed, 0 failed, 3 ignored, `./validate_protocols` 0 tier-1 violations and 0 new tier-2 flags (the one stale baseline entry, `email_opt_in_modal.tsx`, predates the plan), `./validate_migrations` 100 migrations, schema unchanged, and `deno test -A --env-file server/tests/run_progress_test.ts` 4 passed. `./run` skipped: it replaces the machine-global Postgres and Valkey containers serving another session; the Do session substituted the client build and a rolled-back harness, and the harness above repeats the second. |
| 2026-09-24 | 2 | Step 2 reviewed: 1 finding. |
| 2026-09-24 | 3 | Choice the plan did not cover: the chip pulses on `currentModuleId` only while the run is generating. `markRunGenerationFailed` leaves `currentModuleId` on the module that errored, so ruling 5 taken alone would pulse a failed package's error chip. |
| 2026-09-24 | 3 | Choice the plan did not cover: the failed body omits the stage prefix when the stored stage is `ended`. Only the transform writes `ended` on a failed row (the pipeline sets `ended` in memory just before `publishReadyRun`, and `markRunGenerationFailed` re-reads the stored stage, `publishing` at the latest), so "Complete" would sit above "Generation was interrupted by a server restart" on every pre-plan failure. |
| 2026-09-24 | 3 | Choice the plan did not cover: the "Running" row keys on the `module` stage's own `moduleId`, which equals `currentModuleId` in that stage. The bar renders under a non-keyed `Show` so each push updates it in place instead of remounting it. |
| 2026-09-24 | 3 | Review 2's finding: `SYSTEM_08_results_packages.md`'s push count corrected from about thirty to about sixty for a twelve-module run, with the reason. |
| 2026-09-24 | 3 | Noted, outside the surface: `moduleLabel` in `package_view/status.tsx` and the private `moduleLabel` in `lib/run_progress.ts` are the same four lines. Unifying them means exporting the lib one, which step 2 owned; left for an S8 open item. |
| 2026-09-24 | 3 | Deviation, as in step 2: `./run` was not run because it replaces the machine-global `pg` and `valkey-local` containers serving another session; `cd client && npm run build` substituted, green. |
| 2026-09-24 | 3 | Gates green at `e2a3c091`: `deno task typecheck`, `deno task test` (464 passed, 3 ignored), `./validate_protocols` (0 new tier-2 flags, no baseline change; the one stale entry predates the plan), client build. |
| 2026-09-24 | 3 | Step 3 built. |
